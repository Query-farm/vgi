...
| `vgi_result_cache_per_value` | BOOLEAN | true | Per-**value** memoization for the batched LATERAL operator: after dedup, memoize the worker output keyed on the individual input tuple (`v:`-discriminated key), so a fully-warm distinct set serves without the worker (cross-chunk / cross-query / cross-restart value reuse the per-chunk M2 cache misses — chiefly the **expression-arg** case, since a direct-column correlated ref is already delim-deduped). Rides `vgi_result_cache` + the worker's `vgi.cache.*` opt-in; needs `vgi_exchange_input_dedup`. Scalar per-value not yet wired |
