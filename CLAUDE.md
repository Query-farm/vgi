...
| `vgi_worker_cache_dir` | VARCHAR | `""` | Cache directory for immutable `database://` worker packages. Empty follows `vgi_github_cache_dir`, then `${XDG_CACHE_HOME:-~/.cache}/vgi/releases`. Must be exec-capable. See [docs/database-worker-transport.md](docs/database-worker-transport.md) |
| `vgi_worker_cache_max_bytes` | UBIGINT | 5368709120 (5 GiB) | Managed database-worker artifact budget. Opportunistic cleanup evicts least-recently-used, unleased entries to an 80% low-water mark; 0 disables size eviction. |
| `vgi_worker_cache_ttl_seconds` | UBIGINT | 2592000 (30 days) | Evict database-worker artifacts unused for this long; 0 disables TTL eviction. Catalogs and running/pooled workers hold cross-process leases and cannot be evicted. |
| `vgi_worker_package_max_bytes` | UBIGINT | 536870912 (512 MiB) | Maximum stored/compressed `contents` BLOB accepted from a package table. |
| `vgi_worker_package_max_extracted_bytes` | UBIGINT | 1073741824 (1 GiB) | Maximum decompressed size accepted while installing a database worker package. |
| `vgi_worker_package_max_files` | UBIGINT | 10000 | Maximum archive entries accepted while installing a database worker package. |
...
- `database://catalog/schema/table/worker?package_version=version[#sha256=digest]` → **database-package resolver** (native subprocess platforms): query one committed row matching worker/platform/version through a dedicated DuckDB connection, verify the `contents` BLOB's SHA-256, safely extract/copy it into the shared immutable worker cache, then directly execute its entrypoint over the subprocess transport. Identifiers are quoted and row keys bound; URI components are percent-decoded. The content/format/entrypoint-keyed cache uses cross-process shared leases held by catalogs and running/pooled workers, so TTL/LRU/explicit cleanup cannot remove live code. `vgi_worker_package(...)` packages a file with `read_blob()` + `pragma_platform()`. See [docs/database-worker-transport.md](docs/database-worker-transport.md)
...
**Transport policy.** Which of these a user may reach is gated by `vgi_allowed_transports` (narrow-only) and by `enable_external_access=false` (refuses the local ones). `vgi::CheckLocationPolicy` (`src/vgi_location_policy.cpp`) runs at the two places a LOCATION enters VGI — `VgiCatalogAttach` (before any I/O) and `vgi_catalogs()` (bind + InitGlobal). **When adding a transport or changing dispatch, update `ClassifyLocationForPolicy` to match**: it must classify a string as what dispatch will really do with it, and its fall-through is `subprocess` (a `/bin/sh -c`). See [docs/location_policy.md](docs/location_policy.md).
...
|----------|------|-------------|
| `vgi_catalogs(worker_path, bearer_token := …, oauth_refresh_token := …, oauth_profile := …, oauth_cache := …, iroh_* := …)` | Table | List available catalogs from a worker. HTTP authentication reuses ATTACH handlers; discovery defaults `oauth_profile` to `default` (no attached alias), and `oauth_cache` to `vgi_oauth_cache`. See [catalog discovery](docs/catalog-discovery.md). `worker_path` accepts any LOCATION scheme incl. `oci://` (container resolved on first use with default options) and `iroh://` / `httpi://` (same default Iroh config as ATTACH: scoped `TYPE iroh` secret, else ephemeral identity; optional named `iroh_secret_key` / `iroh_relay_urls` / `iroh_no_relay` / `iroh_remote_relay_url` / `iroh_direct_addresses` mirror the ATTACH options via the shared `ApplyIrohOption`). Not `database://` (only ATTACH resolves it). Gated by `vgi_allowed_transports` |
...
| `vgi_worker_package(path, worker_name, package_version, package_format := 'auto', entrypoint := NULL, platform := NULL)` | Table macro | Produce one canonical package-table row using `read_blob()` and `pragma_platform()`, including SHA-256 and `created_at`. Archives require an entrypoint. See [docs/database-worker-transport.md](docs/database-worker-transport.md) |
| `vgi_worker_cache()` | Table | List database worker artifacts: identity/source/digest/format/entrypoint/directory, size, unused age, and whether a cross-process lease is active. |
| `vgi_worker_cache_prune()` | Table | Synchronously apply database-worker cache TTL and size limits. Returns `removed`, `bytes_removed`, `skipped_in_use`. |
| `vgi_worker_cache_flush()` | Table | Attempt to remove all database-worker artifacts. Lease-protected artifacts are retained and counted in `skipped_in_use`. |
...
| `vgi_database_worker.cpp` | `database://` coordinate parser/resolver, parameterized committed-table lookup, SHA-256 verification, immutable atomic artifact install, cross-process leases, last-use metadata, and TTL/LRU/explicit cleanup. See [docs/database-worker-transport.md](docs/database-worker-transport.md) |
