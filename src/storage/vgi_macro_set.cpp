...
static void QualifyWorkerFunctions(ParsedExpression &expression,
	...
	if (expression.GetExpressionClass() == ExpressionClass::FUNCTION) {
		...
		const bool owner_relative =
		    function.catalog.empty() && (function.schema.empty() || StringUtil::CIEquals(function.schema, schema_name));
		if (owner_relative && worker_functions.find(function.function_name) != worker_functions.end()) {
...
void VgiMacroSet::LoadEntries(ClientContext &context, const std::lock_guard<std::mutex> &/*_load_lock*/) {
	...
	auto macros = schema_.GetMacroInventory(rpc_ctx, macro_catalog_type_, context);
	if (macros.empty()) {
		return;
	}
	auto worker_functions = schema_.GetMacroCallableNames(rpc_ctx, ReadTrustEmptyKinds(context), context);
