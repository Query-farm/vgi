...
void NativeDelegationMarkerExecute(ClientContext &, TableFunctionInput &,
                                   DataChunk &) {
  throw InternalException(
      "VgiRequiredFiltersOptimizer did not fire — native-delegation "
      "placeholder "
      "reached execution. Check that the optimizer extension is registered and "
      "that no other pass dropped the marker. This is a bug — please report "
      "it.");
}
...
TableFunction MakeNativeDelegationMarkerFunction() {
  TableFunction fn("vgi_native_delegation_marker", {},
                   NativeDelegationMarkerExecute);
...
VgiTableEntry::VgiTableEntry(Catalog &catalog, SchemaCatalogEntry &schema,
                             CreateTableInfo &info,
                             const vgi::VgiTableInfo &table_info)
    : TableCatalogEntry(catalog, schema, info), table_info_(table_info),
      catalog_(catalog) {
...
unique_ptr<BaseStatistics> VgiTableEntry::GetStatistics(ClientContext &context,
                                                        column_t column_id) {
  if (column_id == COLUMN_IDENTIFIER_ROW_ID ||
      column_id == COLUMN_IDENTIFIER_EMPTY) {
    return nullptr;
  }
  ...
  auto qualifier = ParentSchema().name + "." + name;
  {
    ...
    if (stats_cache_.loading) {
      ...
      wait_ms = std::chrono::duration<double, std::milli>(wait_end - wait_start)
                    .count();
    }
  ...
  std::string outcome =
      must_fetch ? "fetched"
                 : (concurrent_wait ? "concurrent_wait" : "fresh_hit");
...
void VgiTableEntry::FetchColumnStatistics(ClientContext &context) const {
  ...
  try {
    ...
    vgi::CatalogRpcContext rpc_ctx{attach_params,
                                   attach_result_data->attach_opaque_data,
  ...
  try {
    ...
    if (batch && batch->num_rows() > 0) {
      parsed_entries = vgi::ParseColumnStatisticsBatch(
          ...
          vgi_catalog.attach_parameters()->worker_path(), "table",
...
TableFunction
VgiTableEntry::GetScanFunction(ClientContext &context,
...
void VgiTableEntry::RecordMultiBranchHintNoAT(bool is_multi_branch) const {
  ...
  multi_branch_hint_no_at_.compare_exchange_strong(expected, target,
                                                   std::memory_order_acq_rel);
}

vgi::VgiScanBranchesResult
VgiTableEntry::FetchScanBranches(ClientContext &context,
  ...
  vgi::CatalogRpcContext rpc_ctx{attach_params,
                                 attach_result->attach_opaque_data,
                                 vgi_tx_scan.GetTransactionOpaqueData()};
  rpc_ctx.entity_kind = "table";
  rpc_ctx.entity_qualifier = ParentSchema().name + "." + name;
  auto result = vgi::InvokeCatalogTableScanBranchesGet(
      rpc_ctx, ParentSchema().name, name, context, at_unit, at_value);
...
TableFunction VgiTableEntry::GetScanFunctionImpl(
    ClientContext &context, unique_ptr<FunctionData> &bind_data,
  ...
  if (table_info_.scan_function.has_value()) {
    ...
    VGI_LOG(context, "vgi.scan_function.inlined",
            {{"schema", ParentSchema().name},
             {"table", name},
             {"function", scan_result.function_name}});
  } else {
    ...
    if (!at_unit.empty() &&
        (branches_result.branches.size() > 1 ||
         !branches_result.branches[0].branch_filter.empty() ||
         branches_result.branches[0].IsCatalogTable() ||
         branches_result.branches[0].IsFormatBranch())) {
    ...
    const bool single_branch_needs_rewriter =
        branches_result.branches.size() == 1 &&
        (!branches_result.branches[0].branch_filter.empty() ||
         branches_result.branches[0].writable ||
         branches_result.branches[0].IsCatalogTable() ||
         branches_result.branches[0].IsFormatBranch());
    ...
    if (branches_result.branches.size() > 1 || single_branch_needs_rewriter) {
      ...
      if (!context.TryGetCurrentSetting("vgi_multi_branch_scans", mb_enabled) ||
        ...
        throw BinderException(
            "Multi-branch VGI table scan disabled via "
            "vgi_multi_branch_scans=false. "
      ...
      marker_bd->required_extensions =
          std::move(branches_result.required_extensions);
    ...
    scan_result.required_extensions =
        std::move(branches_result.required_extensions);
  }
  ...
  OrderPreservationType resolved_order_preservation =
      OrderPreservationType::INSERTION_ORDER;
  bool order_preservation_set = false;
  std::vector<std::string> scan_filter_semantic_profiles;
  std::vector<vgi::VgiFilterFunctionCapability>
      scan_additional_filter_functions;
  ...
  bool worker_schema_is_third_schema =
      !scan_result.schema_name.empty() &&
      scan_result.schema_name != ParentSchema().name &&
      scan_result.schema_name != default_schema;
  if (worker_schema_is_third_schema) {
    // Worker-provided through VGI 2.0 schema_path, naming a schema other than
    // the table's own or the catalog's default — authoritative, no guessing
    ...
    func_entry = catalog_.GetEntry<TableFunctionCatalogEntry>(
        context, scan_result.schema_name, scan_result.function_name,
    ...
    func_entry = catalog_.GetEntry<TableFunctionCatalogEntry>(
        context, ParentSchema().name, scan_result.function_name,
        OnEntryNotFound::RETURN_NULL);
    if (func_entry) {
      scan_function_schema = ParentSchema().name;
    } else {
      // Function may be in a different schema (e.g., main) than the table's
      // schema (e.g., data)
      if (default_schema != ParentSchema().name) {
        func_entry = catalog_.GetEntry<TableFunctionCatalogEntry>(
            context, default_schema, scan_result.function_name,
            OnEntryNotFound::RETURN_NULL);
        if (func_entry) {
  ...
  if (!func_entry) {
    ...
    EntryLookupInfo lookup(CatalogType::TABLE_FUNCTION_ENTRY,
                           scan_result.function_name);
    auto sys_entry = Catalog::GetEntry(context, SYSTEM_CATALOG, DEFAULT_SCHEMA,
  ...
  if (from_system_catalog && func_entry) {
    ...
    TableFunctionBindInput bind_input(
        ...
        native_tf.function_info.get(), nullptr, native_tf, ref);
    vector<LogicalType> return_types;
    vector<string> return_names;
    auto native_bind =
        native_tf.bind(context, bind_input, return_types, return_names);
    virtual_column_map_t native_virtual_columns;
    if (native_tf.get_virtual_columns) {
      native_virtual_columns =
          native_tf.get_virtual_columns(context, native_bind.get());
    }
    ...
    // misrouting at scan time.
    {
      ...
      if (decl_count != return_names.size()) {
        throw BinderException(
            ...
            "declares "
            "%llu column(s) but the native bind returned %llu. The catalog's "
            "columns "
            "must match exactly what the native function emits at scan time "
            "(positions "
            "+ names). Common cause: Hive-partition columns that read_parquet "
            "appends "
            "but the worker's schema source omitted.",
            catalog_.GetName(), ParentSchema().name, name,
            scan_result.function_name,
      ...
      for (idx_t i = 0; i < decl_count; ++i) {
        ...
        if (decl_name != return_names[i]) {
          throw BinderException(
              ...
              catalog_.GetName(), ParentSchema().name, name,
              scan_result.function_name, static_cast<unsigned long long>(i),
              decl_name, return_names[i]);
        }
      }
    }
    ...
    bind_data = make_uniq<VgiNativeDelegationMarkerBindData>(
        *this, std::move(native_tf), std::move(native_bind),
        std::move(return_types), std::move(return_names),
        std::move(native_virtual_columns), std::move(scan_result),
  ...
  if (func_entry) {
    for (auto &tf : func_entry->functions.functions) {
      has_projection_pushdown =
          has_projection_pushdown || tf.projection_pushdown;
      ...
      if (tf.function_info) {
        ...
        scan_filter_semantic_profiles =
            vgi_tf_info.function_info().filter_semantic_profiles;
        scan_additional_filter_functions =
            vgi_tf_info.function_info().additional_filter_functions;
        ...
        has_late_materialization =
            has_late_materialization ||
            vgi_tf_info.function_info().late_materialization.value_or(false);
      }
    }
  }
  ...
  scan_bind_data->arguments = vgi::BuildArgumentsFromValues(
      context, scan_result.positional_arguments, named_args_vec);
  ...
  scan_bind_data->additional_filter_functions =
      scan_additional_filter_functions;
  ...
  vector<LogicalType> return_types;
  vector<string> names;
  vgi::PerformVgiTableFunctionBind(context, *scan_bind_data, return_types,
                                   names);
  ...
  //     CreateTableInfoFromVgiTable, which filters it out of the ColumnList)
  //     and already erased from names/return_types in
  //     ApplyBindResultToBindData.
  ...
  // but never *fewer* — that's the case that overflows the batch.
  {
    ...
    if (names.size() < scannable_count) {
      throw BinderException("VGI worker returned a bind output_schema with "
                            "%llu column(s) but the catalog "
                            "advertises %llu for '%s.%s.%s' (function '%s'). "
                            "The bind output schema must cover "
                            "every column the table exposes. A mismatch "
                            "usually means the worker's table listing "
                            "and its scan bind disagree — e.g. a proxy "
                            "narrowed one but not the other.",
  ...
  if (table_info_.cardinality_estimate.has_value() ||
      table_info_.cardinality_max.has_value()) {
    if (table_info_.cardinality_estimate.has_value()) {
  ...
  TableFunction func("vgi_table_scan", {}, vgi::VgiTableFunctionScan, nullptr,
                     vgi::VgiTableFunctionInitGlobal,
                     vgi::VgiTableFunctionInitLocal);
  func.projection_pushdown = has_projection_pushdown;
  const auto supports_filter_v2 =
      std::find(scan_filter_semantic_profiles.begin(),
                scan_filter_semantic_profiles.end(),
                "vgi.duckdb.standard.v1") !=
      scan_filter_semantic_profiles.end();
  func.filter_pushdown = has_filter_pushdown && supports_filter_v2;
  ...
  // PhysicalTableScan::dynamic_filters via
  // JoinFilterPushdownInfo::PushInFilter. A VGI catalog table on the probe side
  // of a hash join would never see the build-side InFilter, breaking join key
  // pushdown for the `SELECT … FROM catalog.schema.table` form. The same fix
  // already lives in vgi_table_function_set.cpp for the function-call form;
  // mirror it here. See that file's comment block for the full rationale and
  // the
  ...
  if (table_info_.row_id_column >= 0) {
    ...
    // stay false.
    func.late_materialization = has_late_materialization &&
                                has_filter_pushdown && has_projection_pushdown;
  }

  return func;
}

void VgiTableEntry::VgiTableScanSerialize(
    Serializer &serializer, const optional_ptr<FunctionData> bind_data_p,
  ...
  if (!bind_data.table_entry) {
    ...
    throw SerializationException("vgi_table_scan cannot be serialized without "
                                 "an originating table entry");
  }
...
VgiTableEntry::VgiTableScanDeserialize(Deserializer &deserializer,
  ...
  unique_ptr<FunctionData> bind_data;
  function =
      vgi_entry.GetScanFunctionImpl(context, bind_data, at_unit, at_value);
  return bind_data;
}
...
TableStorageInfo VgiTableEntry::GetStorageInfo(ClientContext &context) {
  ...
  //
  // DuckDB's binder requires matching index_info when validating ON CONFLICT
  // clauses (see bind_insert.cpp — it iterates storage_info.index_info looking
  // for UNIQUE indexes that match the conflict target columns). Without these
  // entries, the binder rejects ON CONFLICT with "no UNIQUE/PRIMARY KEY
  // constraints that refer to this table".
  //
  // These are NOT real indexes — VGI tables have no local storage or ART
  // indexes. They exist solely to satisfy the binder's validation. Actual
  // conflict detection is delegated to the worker, which owns the data and its
  // constraints.
  //
  // Constraint indices from the worker are in Arrow schema space (which
  // includes the row_id column). DuckDB's physical column space excludes
  // virtual columns like row_id. Adjust by shifting indices down when they're
  // after row_id.
  auto adjust_col = [&](int col) -> column_t {
    auto adjusted = col;
    if (table_info_.row_id_column >= 0 && col > table_info_.row_id_column) {
      adjusted--;
    } else if (table_info_.row_id_column >= 0 &&
               col == table_info_.row_id_column) {
...
virtual_column_map_t VgiTableEntry::GetVirtualColumns() const {
  if (rowid_type_ != LogicalType::INVALID) {
    ...
    result.insert(
        {COLUMN_IDENTIFIER_ROW_ID, TableColumn("rowid", rowid_type_)});
    return result;
  }
  return {};
}
