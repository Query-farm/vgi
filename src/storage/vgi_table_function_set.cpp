...
bool SupportsVgiFilterV2(const vgi::VgiFunctionInfo &info) {
  return info.filter_pushdown.value_or(false) &&
         std::find(info.filter_semantic_profiles.begin(),
                   info.filter_semantic_profiles.end(),
...
// This is used for catalog-bound functions where we know which settings the
// catalog registered
std::map<std::string, Value>
ExtractVgiSettings(ClientContext &context,
...
// default; FixedOrder forces Pipeline::IsOrderDependent() -> true so the
// planner serialises the pipeline (single worker emits all rows).
OrderPreservationType
MapOrderPreservation(std::optional<vgi::VgiOrderPreservation> v) {
  if (!v) {
...
VgiTableFunctionSet::VgiTableFunctionSet(Catalog &catalog,
                                         VgiSchemaEntry &schema)
...
static unique_ptr<FunctionData> VgiCatalogTableFunctionBind(
  ...
  auto binding = vgi::ResolveVgiFunctionBinding(context, vgi_info.target(),
  ...
  bind_data->projection_pushdown =
      vgi_info.function_info().projection_pushdown.value_or(false);
  bind_data->filter_semantic_profiles =
      vgi_info.function_info().filter_semantic_profiles;
  bind_data->additional_filter_functions =
      vgi_info.function_info().additional_filter_functions;
  ...
  // KeyValueMetadata.
  bind_data->supports_batch_index =
      vgi_info.function_info().supports_batch_index;

  // supports_splits folds the worker's capability together with the client-side
  // setting, at BIND. Reading the setting here rather than at init is
  // deliberate: a ``SET vgi_split_scans`` between bind and execute would
  // otherwise produce a bind that assumed splits and an init that did not.
  bind_data->supports_splits =
      vgi_info.function_info().supports_splits && VgiSplitScansEnabled(context);
  bind_data->split_token_ttl_seconds =
      vgi_info.function_info().split_token_ttl_seconds;
  ...
  bind_data->arguments =
      vgi::BuildArgumentsFromValues(context, positional_args, named_args);
  ...
  if (!required_settings.empty()) {
    ...
    if (!missing_settings.empty()) {
      ...
      throw BinderException(
          "Function '%s' requires the following settings to be set: %s. "
      ...
      for (int i = 0; i < out_schema->num_fields(); ++i) {
        ...
        if (meta) {
          ...
          if (idx >= 0 && meta->value(idx) == "true") {
            bind_data->partition_column_indices.push_back(
                static_cast<idx_t>(i));
          }
        }
      }
...
static unique_ptr<FunctionData> VgiCatalogTableInOutFunctionBind(
    ClientContext &context, TableFunctionBindInput &input,
    vector<LogicalType> &return_types, vector<string> &names) {
  // Access the VgiTableFunctionInfo attached to this function
  auto &vgi_info = input.info->Cast<vgi::VgiTableFunctionInfo>();
  ...
  params.table_buffering = vgi_info.function_info().function_type ==
                           vgi::VgiFunctionType::TableBuffering;
  params.source_order_dependent =
      vgi_info.function_info().source_order_dependent;
  params.sink_order_dependent = vgi_info.function_info().sink_order_dependent;
  params.requires_input_batch_index =
      vgi_info.function_info().requires_input_batch_index;
  params.projection_pushdown =
      vgi_info.function_info().projection_pushdown.value_or(false);
  ...
  // column names (it reads batch.column("<name>")). Re-parse the arguments
  // schema to recover them (+ varargs flag) so bind can build the input schema
  // by name.
  params.input_from_args = vgi_info.function_info().input_from_args;
  params.stability =
      vgi_info.function_info().stability; // for the map-operator dedup gate
  if (params.input_from_args && vgi_info.function_info().arguments_schema) {
    auto blended_args = vgi::ParseFunctionArgumentSchema(
        context, vgi_info.function_info().arguments_schema);
...
TableFunctionSet
BuildVgiTableFunctionSet(ClientContext &context,
  ...
  for (const auto &func_info : overloads) {
    ...
    if (func_info.arguments_schema) {
      arg_types =
          vgi::ParseFunctionArgumentSchema(context, func_info.arguments_schema);
    }
    ...
    if (arg_types.HasTableInput() || func_info.input_from_args) {
      ...
      if (func_info.input_from_args && func_info.has_finalize) {
        throw InvalidInputException(
            "Function '%s' advertises input_from_args (blended) AND "
            "has_finalize; a blended "
            "RowTransformFunction is map-shaped and cannot have a finalize.",
            func_info.name);
      }
      // Create a table-in-out function
      // Table-in-out functions use LogicalType::TABLE in args and have an
      // in_out_function
      TableFunction table_func(
          arg_types.positional_types, nullptr, VgiCatalogTableInOutFunctionBind,
      ...
      table_func.projection_pushdown =
          func_info.projection_pushdown.value_or(false);
      if (is_buffering) {
      ...
      table_func.order_preservation_type =
          MapOrderPreservation(func_info.order_preservation);
      ...
      table_func.function_info =
          make_uniq<vgi::VgiTableFunctionInfo>(target, func_info);
      ...
      TableFunction table_func(
          arg_types.positional_types, vgi::VgiTableFunctionScan,
          VgiCatalogTableFunctionBind, vgi::VgiTableFunctionInitGlobal,
          vgi::VgiTableFunctionInitLocal);
      table_func.projection_pushdown =
          func_info.projection_pushdown.value_or(false);
      table_func.filter_pushdown = SupportsVgiFilterV2(func_info);
      table_func.sampling_pushdown =
          func_info.sampling_pushdown.value_or(false);
      table_func.order_preservation_type =
          MapOrderPreservation(func_info.order_preservation);
      if (table_func.filter_pushdown) {
        table_func.pushdown_expression = vgi::VgiPushdownExpression;
      ...
      // Register varargs support if the function accepts additional positional
      // arguments
      if (arg_types.has_varargs) {
        table_func.varargs = arg_types.varargs_type;
      }

      // Attach VgiTableFunctionInfo so the bind function can access worker_path
    ...
    // Include both positional and named parameters so duckdb_functions() shows
    // them correctly
...
void VgiTableFunctionSet::LoadEntries(
  ...
  vgi::CatalogRpcContext rpc_ctx{attach_params,
                                 ...
                                 vgi_tx_load.GetTransactionOpaqueData()};
  rpc_ctx.entity_kind = "schema";
  rpc_ctx.entity_qualifier = schema_.name;

  auto function_list = schema_.GetFunctionInventory(
      rpc_ctx, CatalogType::TABLE_FUNCTION_ENTRY, context);
  ...
  for (auto &func_info : function_list) {
    ...
    if (func_info.function_type != vgi::VgiFunctionType::Table &&
        func_info.function_type != vgi::VgiFunctionType::TableBuffering) {
      throw IOException("VGI worker returned '%s' function_type when 'table' "
                        "was requested (function: %s)",
                        vgi::VgiFunctionTypeToString(func_info.function_type),
                        func_info.name);
    }
  ...
  for (const auto &pair : functions_by_name) {
    ...
    vector<FunctionDescription> descriptions;
    auto func_set = vgi::BuildVgiTableFunctionSet(
        context, pair.first, pair.second, target, descriptions);
    ...
    auto function_entry =
        make_uniq_base<StandardEntry, TableFunctionCatalogEntry>(
    ...
    {
      std::lock_guard<std::mutex> __entry_lk(entry_lock_);
