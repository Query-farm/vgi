...
VgiTableFunctionLocalState::VgiTableFunctionLocalState(
    ...
    : ArrowScanLocalState(std::move(current_chunk), ctx),
      ...
      prefetch_slot_(std::make_shared<VgiPrefetchSlot>()), db_(*ctx.db),
      context_(ctx), attach_params_(std::move(attach_params)) {}
...
VgiTableFunctionLocalState::~VgiTableFunctionLocalState() noexcept {
  ...
  if (!done && cancel_enabled) {
    if (auto *dispatcher = FindVgiCancelDispatcher(db_)) {
      ...
      req.state_token =
          !token.empty() ? std::move(token) : prefetch_slot_->last_state_token;
  ...
  if (attach_params_ && attach_params_->use_pool() && connection) {
    ...
    if (auto pooled = connection->ReleaseForPooling()) {
      try {
        ...
        LogWorkerPoolRelease(context_, lf, rr.pooled, rr.skip_reason,
...
static void ApplyBindResultToBindData(
    ClientContext &context, VgiTableFunctionBindData &bind_data,
    BindResult bind_result, const std::string &conn_id, int worker_pid,
    vector<LogicalType> &return_types, vector<string> &names) {
  bind_data.max_processes = 1;
  bind_data.cardinality_estimate = -1;
  bind_data.secret_dependent =
      bind_data.secret_dependent || bind_result.secret_dependent;
  bind_data.bind_result = std::move(bind_result);

  try {
    ArrowSchemaToDuckDBTypes(context, bind_data.bind_result.output_schema,
                             bind_data.c_schema, bind_data.arrow_table,
                             return_types, names);
...

  {
    ...
    fields.emplace_back(
        "num_columns",
        std::to_string(bind_data.bind_result.output_schema->num_fields()));
...
void PerformVgiTableFunctionBind(ClientContext &context,
  ...
  // Log the invocation
  VGI_LOG(
      context, "table_function.bind",
      {{"worker_path", bind_data.worker_path()},
       {"function_name", bind_data.function_name},
       {"num_args", bind_data.arguments.array
                        ? std::to_string(bind_data.arguments.array->length())
                        : "0"}});

  // Validate that arguments type is a struct (defensive check)
  D_ASSERT(!bind_data.arguments.type ||
           bind_data.arguments.type->id() == arrow::Type::STRUCT);
  ...
  if (bind_data.table_entry) {
    ...
    if (tinfo.bind_result.has_value()) {
      try {
        auto resolved_secrets =
            vgi::ExtractVgiSecrets(context, bind_data.required_secrets);
        auto bind_request_bytes = vgi::BuildBindRequestBytes(
            ...
            /*resolved_secrets_provided=*/false, bind_data.worker_path(),
  ...
  auto bind_worker_pid =
      result.connection ? result.connection->GetSubprocessPid().value_or(-1)
                        : -1;
  auto bind_conn_id =
      result.connection ? result.connection->GetConnIdHex() : "";
...
bool HasFilterFunctionCapability(
    const vector<VgiFilterFunctionCapability> &capabilities,
    const string &namespace_name, const string &name, uint64_t version) {
  for (const auto &capability : capabilities) {
    if (capability.namespace_name == namespace_name &&
        capability.name == name && capability.version == version) {
      return true;
    }
  }
  return false;
...
bool ExpressionTreeIsSupported(
  ...
  case ExpressionClass::BOUND_FUNCTION: {
    ...
    if ((name != "&&" && name != "st_intersects_extent") ||
        function.children.size() != 2 ||
        function.return_type.id() != LogicalTypeId::BOOLEAN ||
        !HasFilterFunctionCapability(additional_functions, "duckdb.spatial",
                                     "intersects_extent", 1)) {
      return false;
    }
  ...
  case ExpressionClass::BOUND_OPERATOR: {
    ...
    case ExpressionType::OPERATOR_NOT:
      return op.children.size() == 1 &&
             ExpressionTreeIsSupported(*op.children[0], additional_functions);
    ...
    case ExpressionType::COMPARE_NOT_IN:
      ...
      for (idx_t i = 1; i < op.children.size(); i++) {
        if (op.children[i]->GetExpressionClass() !=
  ...
  case ExpressionClass::BOUND_CAST: {
    ...
    return !cast.try_cast && cast.child->return_type.IsNumeric() &&
           cast.return_type.IsNumeric() &&
...
bool VgiPushdownExpression(ClientContext &context, const LogicalGet &get,
  ...
  if (std::find(bind_data.filter_semantic_profiles.begin(),
                bind_data.filter_semantic_profiles.end(),
                "vgi.duckdb.standard.v1") ==
      bind_data.filter_semantic_profiles.end()) {
    return false;
  }
  bool supported =
      ExpressionTreeIsSupported(expr, bind_data.additional_filter_functions);
  if (supported) {
    ...
    VGI_LOG(context, "table_function.expression_filter_rejected",
            {{"function_name", bind_data.function_name},
             {"expression", expr.ToString()},
             {"reason",
              "expression tree contains unsupported function or node type"}});
  }
  return supported;
}
...
//! Lifetime contract: the TableFilterSet passed to VgiSerializeFilters must
//! outlive the FilterSerializer.
struct JoinKeysInfo {
  string column_name;
  idx_t column_index;
  LogicalType type;
...
static bool VgiAdvisoryFilterCanThrow(const TableFilter &filter) {
  ...
  case TableFilterType::CONJUNCTION_OR: {
    auto &conjunction = filter.filter_type == TableFilterType::CONJUNCTION_AND
                            ? static_cast<const ConjunctionFilter &>(
                                  filter.Cast<ConjunctionAndFilter>())
                            : static_cast<const ConjunctionFilter &>(
  ...
  case TableFilterType::OPTIONAL_FILTER: {
    ...
    return optional.child_filter &&
           VgiAdvisoryFilterCanThrow(*optional.child_filter);
  }
  case TableFilterType::STRUCT_EXTRACT: {
    auto &structure = filter.Cast<StructFilter>();
    return structure.child_filter &&
           VgiAdvisoryFilterCanThrow(*structure.child_filter);
  }
  default:
    return false;
  }
...
public:
  FilterSerializer(
      const string &worker_path, idx_t join_keys_max_bytes,
      bool allow_external_sets = true,
  ...
  yyjson_mut_val *SerializeFilter(const TableFilter &filter, idx_t column_index,
                                  const string &column_name, bool &advisory) {
    auto input = ColumnRef(column_index, column_name);
    return SerializeFilterExpression(filter, input, column_index, column_name,
                                     advisory);
  }

  const vector<Value> &GetValues() const { return values_; }

  const vector<LogicalType> &GetValueTypes() const { return value_types_; }
  const vector<string> &GetValueNames() const { return value_names_; }

  char *WriteJson(yyjson_mut_val *root) {
    return yyjson_mut_val_write(root, 0, nullptr);
  }

  yyjson_mut_doc *GetDoc() { return doc_; }

  bool HasJoinKeys() const { return !join_key_columns_.empty(); }
...
private:
  ...
  yyjson_mut_val *SerializeFilterExpression(const TableFilter &filter,
    ...
    case TableFilterType::CONSTANT_COMPARISON: {
      ...
      if (string(op) == "unknown") {
        throw InvalidInputException(
            "Unsupported VGI constant comparison operator");
      }
      auto obj = yyjson_mut_obj(doc_);
    ...
    case TableFilterType::IN_FILTER: {
      ...
      if (!allow_external_sets_ || estimated_bytes <= INLINE_SET_MAX_BYTES) {
        ...
        yyjson_mut_obj_add_uint(doc_, set, "value_ref",
                                AddValue(Value::LIST(in_filter.values[0].type(),
                                                     in_filter.values)));
      } else {
        ...
        yyjson_mut_obj_add_strcpy(doc_, set, "column_name",
                                  column_name.c_str());
        join_key_columns_.push_back({column_name, column_index,
                                     in_filter.values[0].type(),
                                     &in_filter.values});
      }
    ...
    case TableFilterType::CONJUNCTION_OR: {
      ...
      for (auto &child : conjunction.child_filters) {
        ...
        auto child_expr = SerializeFilterExpression(
            *child, ColumnRef(column_index, column_name), column_index,
      ...
      yyjson_mut_obj_add_str(
          doc_, obj, "node",
          filter.filter_type == TableFilterType::CONJUNCTION_AND ? "and"
                                                                 : "or");
    ...
    case TableFilterType::STRUCT_EXTRACT: {
      ...
      yyjson_mut_obj_add_uint(doc_, field, "field_index",
                              struct_filter.child_idx);
      yyjson_mut_obj_add_strcpy(doc_, field, "field_name",
                                struct_filter.child_name.c_str());
      return SerializeFilterExpression(*struct_filter.child_filter, field,
                                       column_index, column_name, advisory);
    }
    case TableFilterType::EXPRESSION_FILTER: {
      auto &expr_filter = filter.Cast<ExpressionFilter>();
      if (!ExpressionTreeIsSupported(*expr_filter.expr,
                                     additional_functions_)) {
        throw InvalidInputException("VGI required expression filter is not "
                                    "eligible for v2 serialization");
      }
    ...
    case TableFilterType::OPTIONAL_FILTER: {
      ...
      if (!optional_filter.child_filter ||
          VgiContainsDynamicFilter(*optional_filter.child_filter)) {
        return nullptr;
      }
      return SerializeFilterExpression(*optional_filter.child_filter, input,
                                       column_index, column_name, advisory);
    }
    case TableFilterType::DYNAMIC_FILTER:
    case TableFilterType::BLOOM_FILTER:
      ...
      return nullptr;
    default:
      throw InvalidInputException("VGI filter pushdown failed for worker '%s': "
                                  "unknown filter type %d cannot be serialized",
                                  worker_path_,
  ...
  yyjson_mut_val *SerializeExpression(const Expression &expr,
    ...
    case ExpressionClass::BOUND_REF: {
      ...
      if (ref.index != 0) {
        throw InvalidInputException(
            "VGI DuckDB 1.5 filter references more than one column");
      }
    ...
    case ExpressionClass::BOUND_FUNCTION: {
      ...
      if ((name != "&&" && name != "st_intersects_extent") ||
          function.children.size() != 2 ||
          !HasFilterFunctionCapability(additional_functions_, "duckdb.spatial",
                                       "intersects_extent", 1)) {
        break;
      }
      ...
      for (const auto &child : function.children) {
        yyjson_mut_arr_append(
            arguments, SerializeExpression(*child, column_index, column_name));
      }
    ...
    case ExpressionClass::BOUND_COMPARISON: {
      ...
      yyjson_mut_obj_add_val(
          doc_, obj, "left",
          SerializeExpression(*comparison.left, column_index, column_name));
      yyjson_mut_obj_add_val(
          doc_, obj, "right",
    ...
    case ExpressionClass::BOUND_CONJUNCTION: {
      ...
      if (conjunction.children.size() < 2) {
        throw InvalidInputException(
            "VGI boolean conjunction requires at least two children");
      }
      yyjson_mut_obj_add_str(doc_, obj, "node",
                             conjunction.GetExpressionType() ==
      ...
      for (auto &child : conjunction.children) {
        yyjson_mut_arr_append(
            children, SerializeExpression(*child, column_index, column_name));
      }
      yyjson_mut_obj_add_val(doc_, obj, "children", children);
      return obj;
    }
      ...
      if ((op.GetExpressionType() == ExpressionType::COMPARE_IN ||
        ...
        for (idx_t i = 1; i < op.children.size(); i++) {
          if (op.children[i]->GetExpressionClass() !=
              ExpressionClass::BOUND_CONSTANT) {
            throw InvalidInputException(
                "VGI IN expression candidates must be constants");
          }
          values.push_back(
              op.children[i]->Cast<BoundConstantExpression>().value);
        }
        yyjson_mut_obj_add_str(doc_, obj, "node", "in");
        yyjson_mut_obj_add_val(
            doc_, obj, "expression",
    ...
    case ExpressionClass::BOUND_CAST: {
      auto &cast = expr.Cast<BoundCastExpression>();
      if (cast.try_cast || !cast.child->return_type.IsNumeric() ||
          !cast.return_type.IsNumeric()) {
        break;
      }
    ...
    throw InvalidInputException(
        "VGI expression filter serialization failed for worker '%s': "
        "unsupported expression class %d",
  ...
  static idx_t EstimateJoinKeyBytes(const vector<Value> &values) {
    ...
    if (type_size > 0 &&
        values.size() > std::numeric_limits<idx_t>::max() / type_size) {
      return std::numeric_limits<idx_t>::max();
    }
...
SerializedFilters VgiSerializeFilters(
  ...
  Value max_bytes_val;
  if (context.TryGetCurrentSetting("vgi_join_keys_max_bytes", max_bytes_val) &&
      !max_bytes_val.IsNull()) {
  ...
  FilterSerializer serializer(worker_path, join_keys_max_bytes, true,
                              additional_functions);
  auto document = yyjson_mut_obj(serializer.GetDoc());
  yyjson_mut_obj_add_str(serializer.GetDoc(), document, "encoding",
                         "vgi.filters.v2");
  yyjson_mut_obj_add_str(serializer.GetDoc(), document, "semantics",
                         "vgi.duckdb.standard.v1");
  ...
  for (auto &entry : filters->filters) {
    ...
    // predicate keys so the residual filter reflects only non-partition
    // constraints.
    ...
    if (index_domain == VgiFilterColumnIndexDomain::BIND_SCHEMA) {
      ...
      if (col_idx >= column_ids.size()) {
        throw InvalidInputException(
            "VGI filter column %llu has no projected-to-bind-schema mapping "
            "for worker '%s'",
    ...
    // sentinel (UINT64_MAX). Without remapping, col_name would become the
    // literal "18446744073709551615" and the worker — which resolves
    // pushed/join-key columns by name — would silently drop it (the
    // late-materialization semi-join still guarantees correctness, but the
    // rowid IN-list / min-max pushdown is lost). Resolve to the worker's actual
    // rowid field name (passed explicitly because column_names has the rowid
    // column erased — indexing it would mis-resolve).
    string col_name;
    idx_t worker_col_idx;
    if (original_col_idx == COLUMN_IDENTIFIER_ROW_ID &&
        !rowid_column_name.empty()) {
      col_name = rowid_column_name;
      if (rowid_worker_col_index < 0) {
        throw InvalidInputException(
            "VGI rowid filter has no worker-schema index for worker '%s'",
            worker_path);
      }
      worker_col_idx = NumericCast<idx_t>(rowid_worker_col_index);
    } else {
      if (original_col_idx == COLUMN_IDENTIFIER_ROW_ID ||
          original_col_idx >= column_names.size()) {
        throw InvalidInputException(
            "VGI filter bind-schema column %llu is invalid for worker '%s'",
    ...
    auto append_filter = [&](const TableFilter &candidate,
                             bool inherited_advisory) {
      bool advisory =
          inherited_advisory ||
          candidate.filter_type == TableFilterType::OPTIONAL_FILTER ||
          candidate.filter_type == TableFilterType::DYNAMIC_FILTER ||
          candidate.filter_type == TableFilterType::BLOOM_FILTER;
      try {
        auto expression = serializer.SerializeFilter(candidate, worker_col_idx,
                                                     col_name, advisory);
        if (!expression) {
          if (advisory) {
            return;
          }
          throw InvalidInputException("VGI required filter for worker '%s' "
                                      "could not be serialized atomically",
                                      worker_path);
        }
        auto predicate = yyjson_mut_obj(serializer.GetDoc());
        auto id = string(advisory ? "advisory:" : "query:") +
                  std::to_string(predicate_index++);
        yyjson_mut_obj_add_strcpy(serializer.GetDoc(), predicate, "id",
                                  id.c_str());
        yyjson_mut_obj_add_uint(serializer.GetDoc(), predicate, "revision", 0);
        yyjson_mut_obj_add_str(serializer.GetDoc(), predicate, "mode",
                               advisory ? "advisory" : "required");
        yyjson_mut_obj_add_str(serializer.GetDoc(), predicate, "source",
                               advisory ? "other" : "query");
        yyjson_mut_obj_add_val(serializer.GetDoc(), predicate, "expression",
                               expression);
  ...
  yyjson_mut_obj_add_val(serializer.GetDoc(), document, "predicates",
                         predicates);
  ...
  if (!import_result.ok()) {
    throw IOException("Failed to import filter RecordBatch: %s",
  ...
  new_fields.push_back(
      arrow::field("filter_spec", arrow::utf8(), /*nullable=*/false));
  ...
  record_batch = arrow::RecordBatch::Make(new_schema, record_batch->num_rows(),
                                          record_batch->columns());

  // Serialize to Arrow IPC stream format (schema + record batch)
  auto buffer_result = arrow::io::BufferOutputStream::Create();
  if (!buffer_result.ok()) {
    throw IOException("Failed to create buffer for filter IPC: %s",
                      buffer_result.status().ToString());
  }
  auto buffer_stream = buffer_result.ValueUnsafe();

  auto writer_result =
      arrow::ipc::MakeStreamWriter(buffer_stream, record_batch->schema());
  if (!writer_result.ok()) {
    throw IOException("Failed to create IPC stream writer: %s",
  ...
  if (!write_status.ok()) {
    throw IOException("Failed to write filter RecordBatch to IPC stream: %s",
                      write_status.ToString());
  }

  auto close_status = writer->Close();
  if (!close_status.ok()) {
    throw IOException("Failed to close IPC stream writer: %s",
                      close_status.ToString());
  }

  auto finish_result = buffer_stream->Finish();
  if (!finish_result.ok()) {
    throw IOException("Failed to finish filter IPC buffer: %s",
  ...
  // Each IN filter gets its own batch, so different cardinalities are
  // supported.
  ...
  if (serializer.HasJoinKeys()) {
    ...
    for (auto &kc : key_columns) {
      ...
      auto metadata =
          arrow::KeyValueMetadata::Make({"vgi_join_keys_version"}, {"2"});
      key_batch = key_batch->ReplaceSchemaMetadata(metadata);

      // Serialize to Arrow IPC
      auto buf_result = arrow::io::BufferOutputStream::Create();
      if (!buf_result.ok()) {
        throw IOException("Failed to create buffer for join keys IPC: %s",
                          buf_result.status().ToString());
      }
      auto buf_stream = buf_result.ValueUnsafe();

      auto writer_res =
          arrow::ipc::MakeStreamWriter(buf_stream, key_batch->schema());
      if (!writer_res.ok()) {
        throw IOException("Failed to create IPC writer for join keys: %s",
      ...
      if (!write_status.ok()) {
        throw IOException("Failed to write join keys RecordBatch: %s",
                          write_status.ToString());
      }
      auto close_status = writer->Close();
      if (!close_status.ok()) {
        throw IOException("Failed to close join keys IPC writer: %s",
                          close_status.ToString());
      }
      auto finish_res = buf_stream->Finish();
      if (!finish_res.ok()) {
        throw IOException("Failed to finish join keys IPC buffer: %s",
...
// both compute the key byte-identically). They are reached here via the
// header's duckdb::vgi declarations.
...
struct CacheEligibility {
  ...
  VgiResultCacheKey
      key; //! populated when eligible (catalog-scoped: txn_id empty)
  std::string catalog_name;
  bool catalog_version_frozen = false;
  //! Current transaction identifier. The KEY stays catalog-scoped (txn_id
  //! empty) because scope is worker-advertised on the first batch, not known
  //! here — but a `scope=transaction` result folds this into its key at commit
  //! and a lookup probes both the catalog key and (key + this txn_id).
  std::string transaction_id;

  // --- Per-partition cache classification (SINGLE_VALUE_PARTITIONS) ---------
  //! Splittable: eligible to split the captured result by partition value AND
  //! (if enumerable) serve a `=`/`IN` scan from per-partition entries. False
  //! when the function isn't partition-scope-eligible, the setting is off, or a
  //! partition column carries a non-`=`/`IN` (range/other) predicate. When
  //! false the whole per-partition layer is skipped (the whole-scan cache still
  //! operates normally).
  bool partition_splittable = false;
  //! Enumerable: every partition column is constrained by `=`/`IN`, so the
  //! requested partition set is finite and known here (in
  //! `partition_enumerated_tuples`). Only an enumerable scan can be SERVED from
  //! per-partition entries; a non-enumerable splittable scan only POPULATES
  //! them.
  bool partition_enumerable = false;
  std::vector<std::vector<Value>>
      partition_enumerated_tuples;             // parallel to partition_types
  std::string partition_residual_filter_bytes; // filter_bytes minus the
                                               // partition predicate
  std::vector<idx_t>
...
//! Resolve partition column names (from the Arrow bind output schema — the
//! index space of `partition_column_indices`, which INCLUDES the rowid column)
//! and their DuckDB types (matched BY NAME into
//! all_column_names/all_column_types, whose index space has the rowid erased).
//! Returns false if any partition column can't be resolved. Keeps
//! `names`/`types` in declared partition order.
...
//! Returns true iff `filter` constrains its (single) column to a FINITE
//! enumerable set of constants — an `=`, an `IN`, or an `OR` of those —
//! appending the constants to `out`. Returns false for any range / NULL / AND /
//! other shape (not enumerable). A TableFilterSet entry is per-column, so an OR
//! here is `col=a OR col=b` on one column.
static bool ExtractPartitionEqualityValues(const TableFilter &filter,
  ...
  case TableFilterType::OPTIONAL_FILTER: {
    // DuckDB wraps a pushed `col IN (...)` in an OptionalFilter ("not required
    // for query correctness" — enforced above the scan too). Its child is the
    // real IN/=. Any dynamic filter was already rejected upstream
    // (VgiContainsDynamicFilter → cache-ineligible), so an optional reaching
    // here is static and safe to unwrap.
    auto &opt = filter.Cast<OptionalFilter>();
    return opt.child_filter &&
           ExtractPartitionEqualityValues(*opt.child_filter, out);
  }
  default:
...
//! Classify the pushed filter for per-partition caching, filling the
//! partition_* fields of `e`. Called only when the base scan is already
//! cache-eligible (so dynamic filters were rejected upstream). Sets
//! `partition_splittable` (may populate per-partition entries) and, when every
//! partition column is `=`/`IN`-constrained, `partition_enumerable`
//! + `partition_enumerated_tuples` (may serve). A non-`=`/`IN` (range/other)
//! predicate on ANY partition column disables the whole per-partition layer
//! (leaves it false).
static void ClassifyPartitionFilters(ClientContext &context,
  ...
  Value pv;
  if (context.TryGetCurrentSetting("vgi_result_cache_partition_scope", pv) &&
      !pv.IsNull() && !pv.GetValue<bool>()) {
    return;
  }
  // Ordered/sampled scans can't use the combined per-partition serve
  // (concatenation can't honor a global ORDER BY; per-partition sampling
  // semantics differ), so the whole per-partition layer is skipped — the
  // whole-scan cache handles them.
  if (bind_data.order_by_hint || bind_data.table_sample_hint) {
    return;
  }
  Value maxv;
  if (context.TryGetCurrentSetting("vgi_result_cache_partition_max_enumerated",
                                   maxv) &&
      !maxv.IsNull()) {
    e.partition_max = maxv.GetValue<uint64_t>();
  }
  if (!ResolvePartitionColumns(bind_data, e.partition_names,
                               e.partition_types)) {
    return;
  }
  ...
  std::set<idx_t>
      exclude_keys; // projected filter-map keys that are partition predicates
  ...
  if (input.filters) {
    for (auto &entry : input.filters->filters) {
      ...
      auto &filter = *entry.second;
      const idx_t original_col_idx = col_idx < input.column_ids.size()
                                         ? input.column_ids[col_idx]
                                         : col_idx;
      std::string col_name;
      if (original_col_idx == COLUMN_IDENTIFIER_ROW_ID &&
          !bind_data.rowid_column_name.empty()) {
  ...
  // Residual = the pushed filters MINUS the partition predicates. CRITICAL:
  // when EVERY pushed filter is a partition predicate (all excluded), the
  // residual must be the empty string — byte-identical to a full/no-filter
  // scan's residual — so a `WHERE part=X` scan reuses the per-partition entry a
  // full scan stored. Serializing an all-excluded set would still emit a
  // non-empty empty-filter IPC batch and never match. So only serialize when a
  // NON-excluded filter remains.
  ...
  if (has_residual_filter) {
    try {
      auto sf = VgiSerializeFilters(
          context, input.column_ids, input.filters, bind_data.all_column_names,
          bind_data.worker_path(), bind_data.rowid_column_name,
          bind_data.rowid_worker_col_index, &exclude_keys,
          VgiFilterColumnIndexDomain::PROJECTED,
          bind_data.additional_filter_functions);
      if (sf.filter_bytes) {
        e.partition_residual_filter_bytes.append(
            reinterpret_cast<const char *>(sf.filter_bytes->data()),
            static_cast<size_t>(sf.filter_bytes->size()));
      }
      for (auto &jk : sf.join_keys_buffers) {
        e.partition_residual_filter_bytes.push_back('|');
        e.partition_residual_filter_bytes.append(
            reinterpret_cast<const char *>(jk->data()),
  ...
  if (!any_partition_pred) {
    return; // full/partition-free scan: populate-only (can't enumerate the
            // universe)
  }
...
//! The "p:"-prefixed input_hash discriminator for one partition-value tuple.
//! Both the serve side (filter constants) and the capture side (decoded
//! partition min-values) call this with the same `partition_types`, so
//! identical tuples produce identical discriminators (the per-partition-cache
//! correctness linchpin).
static std::string
PartitionDiscriminator(ClientContext &context, const std::vector<Value> &tuple,
                       const std::vector<LogicalType> &partition_types) {
  return "p:" + VgiSha256Hex(vgi::CanonicalPartitionTupleKey(context, tuple,
                                                             partition_types));
}

//! Build a per-partition cache key from the base (whole-scan) key: swap
//! filter_bytes for the residual (partition predicate stripped), set the "p:"
//! input_hash discriminator, and clear order/sample/transaction dimensions
//! (partition entries are catalog-scoped, order/sample-free by construction).
static VgiResultCacheKey BuildPartitionKey(const VgiResultCacheKey &base,
...
//! Human-readable "name=value, name2=value2" label of a partition tuple
//! (diagnostics).
static std::string PartitionLabel(const std::vector<std::string> &names,
...
CacheEligibility EvaluateCacheEligibility(
    ClientContext &context, const VgiTableFunctionBindData &bind_data,
    TableFunctionInitInput &input, const std::vector<int32_t> &projection_ids) {
  CacheEligibility e;
  // Global master switch.
  Value master;
  if (context.TryGetCurrentSetting("vgi_result_cache", master) &&
      !master.GetValue<bool>()) {
  ...
  // Secret values are intentionally neither serialized into the key nor
  // retained by the cache. Any declared secret dependency makes the result
  // sensitive to rotation/removal, so probing or storing it would serve stale
  // credentials.
  ...
  if (bind_data.table_entry) {
    ...
    auto &catalog =
        const_cast<Catalog &>(bind_data.table_entry->ParentCatalog());
    ...
    identity_scope = vgi::BuildCatalogIdentityScope(
        catalog_label, bind_data.attach_params->auth());
  ...
  if (bind_data.table_sample_hint) {
    ...
    sample_key =
        std::to_string(s.sample_percentage) + "/" + std::to_string(s.seed);
  }
  ...
  e.key.canonical_arguments =
      bind_data.arguments.array ? bind_data.arguments.array->ToString() : "";
  ...
  // The key stays catalog-scoped here (txn_id empty); the current transaction
  // id is carried separately for the scope=transaction commit + two-key lookup.
  e.transaction_id = std::to_string(
      static_cast<uint64_t>(context.ActiveTransaction().global_transaction_id));
  e.catalog_name = catalog_label;
  // Per-partition classification piggybacks on the base eligibility (same
  // identity / version / opt-out). Populates the partition_* fields; harmless
  // when the function isn't SINGLE_VALUE_PARTITIONS or the setting is off.
...
// Drain a RAM substream's buffered batches into the spill writer (freeing them
// as it goes). Returns false if the writer refused (per-entry disk budget
// exceeded).
static bool DrainSubstreamToWriter(CachedStream &s, VgiCaptureDiskWriter &w) {
  ...
  for (auto &cb : s.batches) {
    if (ok && cb.ipc) {
      ...
      // RecordBatch is gone), so transcode to the writer's codec here.
      // logical_len is the uncompressed size (no-op transcode when
      // codec=="none").
      auto payload = TranscodeIpcWithCodec(cb.ipc, w.CodecName(), w.Level());
      ok = w.AppendBatch(cb.has_batch_index, cb.batch_index, cb.rows,
                         cb.partition_values_bytes, payload->data(),
...
// ConvertPartitionValuesBatch; forward-declared here so the commit dtor can
// call it.
static void SplitAndStorePartitionEntries(ClientContext *ctx,
                                          VgiResultCaptureCtx &cap,
...
VgiTableFunctionGlobalState::~VgiTableFunctionGlobalState() {
  ...
  auto &cap = *capture;
  // [S6] Release this capture's in-flight budget on EVERY teardown path
  // (commit, abort, incomplete) — the transient capture buffers are done being
  // resident (either committed into the LRU, whose bytes are accounted
  // separately, or freed). Do it first so no early-return leaks the
  // reservation.
  int64_t reserved =
      cap.reserved_inflight_bytes.exchange(0, std::memory_order_relaxed);
  if (reserved > 0) {
  ...
  // Best-effort store diagnostics. The dtor runs during query teardown while
  // the ClientContext is still alive (same assumption as dynamic_to_string);
  // guard + swallow.
  auto log_store = [&](const string &event,
                       const vector<pair<string, string>> &fields) {
  ...
  const bool splits_all_drained =
      !cap.split_path ||
      (cap.total_splits > 0 && cap.completed_splits.load() == cap.total_splits);
  const bool complete = cap.cc_seen && cap.cc.Cacheable() &&
                        !cap.aborted.load() && cap.launched.load() > 0 &&
                        cap.eos.load() == cap.launched.load() &&
                        splits_all_drained;
  // v1: transaction-scoped results are not cached (transaction keying is a
  // later milestone). Refuse rather than mis-key.
  if (!complete) {
    abort_stream();
    return;
  }
  // scope=transaction: reusable only within THIS transaction. Fold the txn id
  // into the key so a different transaction (different id) misses it. Refuse if
  // we have no txn id to isolate by (shouldn't happen).
  ...
  int64_t ttl = cap.cc.ttl_seconds.value_or(
      static_cast<int64_t>(cap.default_ttl_seconds));
  ...
  if (ttl <= 0 && cap.cc.expires_rfc3339.empty()) {
    if (cap.catalog_version_frozen || !cap.key.at_value.empty()) {
      ...
      skip("no_freshness");
      abort_stream();
      return; // no freshness basis — refuse (shouldn't happen: Cacheable()
              // gate)
    }
  }
  entry->expires_at =
      entry->stored_at + std::chrono::seconds(ttl > 0 ? ttl : 0);
  ...
  if (cap.streaming()) {
    ...
    // (ttl=0 + etag + revalidatable, expires_at == stored_at) would therefore
    // be written un-loadable (both disk loaders reject `now >= expires`) and
    // orphan one blob per scan. Refuse to spill it — recompute is the correct
    // behavior.
    ...
    // Transaction-scoped entries are memory-only (the txn id is process-local
    // and ephemeral; a disk blob would orphan after the txn and be meaningless
    ...
    // substreams whose producer finished BEFORE the spill (their RAM batches
    // were never streamed) — single-threaded here, all producers done. Then
    // finalize (hash → rename → ref); the entry is disk-only, discovered via
    // its ref and adopted into memory on a small serve. A failed drain/commit
    // caches nothing.
    for (auto &sp : cap.streams) {
      if (!sp->batches.empty() &&
          !DrainSubstreamToWriter(*sp, *cap.disk_writer)) {
        skip("drain_failed");
        VgiResultCache::Instance().AbortStreamingCapture(*cap.disk_writer);
        return;
      }
    }
    if (VgiResultCache::Instance().CommitStreamingCapture(*cap.disk_writer,
                                                          *entry)) {
      log_store("result_cache.store",
                {{"function", cap.key.function_name},
    ...
    // A spilled capture is disk-only and its RAM batches were drained — no
    // per-batch bytes remain to split by partition. Skip the per-partition
    // layer (v1 boundary).
  ...
  // Per-partition split (additive): store one entry per distinct partition
  // value BEFORE the whole-scan move consumes the substreams. Gated on the
  // worker's first-batch opt-in (cc.partition_scope) and skipped for
  // transaction-scoped results (their key is process-local; partition entries
  // are catalog-scoped). Copies batch shared_ptrs. MEMORY-ONLY
  // (allow_disk=false): the combined serve probes only the memory tier
  // (LookupBatch), so a disk copy would never be read back — and persisting an
  // input_hash-keyed entry would wrongly route it into the packed EXCHANGE
  // store + its ref-count cap. The whole-scan entry above still persists to
  // disk for cross-process warmth of full/identical-filter repeats.
  if (cap.partition_split_requested && cap.cc.partition_scope && !txn_scoped) {
    SplitAndStorePartitionEntries(client_context_for_explain, cap, *entry,
                                  /*allow_disk=*/false);
  }
  // RAM-buffered path (disk tier off): move the substreams in + insert into
  // memory.
  int64_t rows = 0;
  int64_t bytes = 0;
  for (auto &sp : cap.streams) {
  ...
  // Transaction-scoped entries never persist to disk (ephemeral, process-local
  // id).
  if (VgiResultCache::Instance().Insert(std::move(entry),
                                        /*allow_disk=*/!txn_scoped)) {
    ...
    skip("too_large_for_memory"); // > max_entry_bytes with disk off (or
                                  // memory-only txn)
  }
}
...
VgiTableFunctionInitGlobal(ClientContext &context,
  ...
  // Read TABLESAMPLE pushdown from DuckDB optimizer (SYSTEM_SAMPLE percentage
  // only)
  if (input.sample_options) {
    double pct = input.sample_options->sample_size.GetValue<double>();
    int64_t seed =
        input.sample_options->seed.IsValid()
            ? static_cast<int64_t>(input.sample_options->seed.GetIndex())
            : -1;
  ...
  if (bind_data.projection_pushdown) {
    ...
    for (auto col_id : input.column_ids) {
      if (col_id == COLUMN_IDENTIFIER_ROW_ID &&
          bind_data.rowid_worker_col_index >= 0) {
        // Map rowid request to the actual worker column index
        projection_ids.push_back(
            static_cast<int32_t>(bind_data.rowid_worker_col_index));
  ...
  // any cap / the disk dir / the packed backend takes effect. Shared with the
  // exchange path (single source of truth — no drift). [S1] ConfigureIfChanged
  // skips the global lock + evict entirely when the settings are unchanged (the
  // steady state).
  SyncResultCacheSettings(context);
  auto cache_eval =
      EvaluateCacheEligibility(context, bind_data, input, projection_ids);
  ...
  if (cache_eval.eligible) {
    ...
    auto reval = split_path ? nullptr
                            : VgiResultCache::Instance().LookupForRevalidation(
                                  cache_eval.key, now_tp);
    if (reval) {
      ...
      uint64_t reval_min_bytes =
          context.TryGetCurrentSetting("vgi_result_cache_revalidate_min_bytes",
                                       rmv) &&
                  !rmv.IsNull()
    ...
    if (!revalidation_entry) {
      ...
      // Per-partition serve (additive): the whole-scan key missed, but if this
      // scan enumerates its requested partition set (`=`/`IN` on every
      // partition column) AND every per-partition entry is an in-memory hit,
      // serve the union without the worker. Row-exact: each entry keys on the
      // same residual filter, the enumerated set equals the requested set, and
      // SINGLE_VALUE guarantees each cached row belongs to exactly its entry's
      // partition (no super/subset).
      if (cache_eval.partition_enumerable &&
          !cache_eval.partition_enumerated_tuples.empty()) {
        ...
        for (auto &tuple : cache_eval.partition_enumerated_tuples) {
          auto disc = PartitionDiscriminator(context, tuple,
                                             cache_eval.partition_types);
          pkeys.push_back(BuildPartitionKey(
              cache_eval.key, cache_eval.partition_residual_filter_bytes,
              disc));
        }
        ...
        int64_t missing = 0;
        for (auto &h : hits) {
          // Combined serve is memory-only: a disk-backed (streaming) entry
          // can't be aggregated into one replay stream, so treat it as a miss.
          if (!h || h->disk_backed) {
            all_hit = false;
            ++missing;
          }
        ...
        if (all_hit) {
          // Aggregate every partition entry's batches into ONE
          // partition-contiguous replay stream with fresh sequential
          // batch_index — so the sort in CachedReplayConnection keeps each
          // partition's batches together (required by
          // PhysicalPartitionedAggregate) regardless of the stored indices.
          ...
          combined->never_expires =
              true; // transient serve entry (not inserted)
          CachedStream merged;
          uint64_t seq = 0;
        ...
        // fall through to the worker scan (which repopulates per-partition
        // entries).
      }
    }
  ...
  auto serialized_filters = VgiSerializeFilters(
      context, input.column_ids, input.filters, bind_data.all_column_names,
      bind_data.worker_path(), bind_data.rowid_column_name,
      bind_data.rowid_worker_col_index, nullptr,
      VgiFilterColumnIndexDomain::PROJECTED,
      bind_data.additional_filter_functions);
  if (serialized_filters.filter_bytes) {
    VGI_LOG(context, "table_function.filters_serialized",
            {{"function_name", bind_data.function_name},
             {"filter_bytes_size",
              std::to_string(serialized_filters.filter_bytes->size())}});
  }
  if (!serialized_filters.join_keys_buffers.empty()) {
    ...
    VGI_LOG(context, "table_function.join_keys_serialized",
            {{"function_name", bind_data.function_name},
             {"join_keys_count",
              std::to_string(serialized_filters.join_keys_buffers.size())},
  ...
  if (input.filters) {
    ...
    auto try_capture_from_optional = [&](idx_t original_col_idx,
      ...
      } else if (opt.child_filter->filter_type ==
                 TableFilterType::CONJUNCTION_OR) {
        // NULLS_FIRST case: OptionalFilter(ConjunctionOr(IsNull,
        // DynamicFilter))
    ...
    for (auto &entry : input.filters->filters) {
      ...
      idx_t original_col_idx = col_idx < input.column_ids.size()
                                   ? input.column_ids[col_idx]
                                   : col_idx;
      ...
      if (filter.filter_type == TableFilterType::OPTIONAL_FILTER) {
        try_capture_from_optional(worker_col_idx, col_name,
                                  filter.Cast<OptionalFilter>());
  ...
  // This is shared between the global state (which updates it from
  // DynamicFilterData) and the connection (which reads it when building tick
  // batches).
  ...
  // Split path: plan the scan into named units BEFORE any reader starts, then
  // let readers claim them. The plan is built from STATIC filters only —
  // dynamic join-key values are not initialized yet at this point, and continue
  // to reach the worker per-tick exactly as they do on the non-split path. That
  // is not a regression (today nothing is pruned at split granularity either
  // way), just an optimization not claimed here.
  if (bind_data.supports_splits) {
    ...
    if (bind_data.split_token_ttl_seconds.has_value()) {
      Value min_ttl_value;
      int64_t min_ttl = 30;
      if (context.TryGetCurrentSetting("vgi_split_token_min_ttl_seconds",
                                       min_ttl_value) &&
      ...
      if (min_ttl > 0 && declared < min_ttl) {
        throw InvalidInputException(
            "VGI function '%s' declares a split-token lifetime of %lld "
            "second(s), but this client "
            "needs at least %lld between planning a scan and reading it. "
            "Nothing re-plans when a "
            "split token expires, so the scan would fail partway rather than "
            "degrade. Raise "
            "vgi_split_token_min_ttl_seconds if that horizon is wrong for this "
            "deployment, or fix "
    ...
    if (filter_bytes) {
      filter_bytes_vec.assign(filter_bytes->data(),
                              filter_bytes->data() + filter_bytes->size());
    }
    ...
    const int64_t min_splits = static_cast<int64_t>(
        TaskScheduler::GetScheduler(context).NumberOfThreads());
    int64_t target_split_bytes = 0;
    Value tsb;
    if (context.TryGetCurrentSetting("vgi_target_split_bytes", tsb) &&
        !tsb.IsNull()) {
      target_split_bytes = static_cast<int64_t>(tsb.GetValue<uint64_t>());
    }

    CatalogRpcContext rpc_ctx{bind_data.attach_params,
                              bind_data.attach_opaque_data,
                              bind_data.transaction_opaque_data};
    auto plan = InvokeTableFunctionPlan(
        rpc_ctx, bind_data.bind_result.bind_request_bytes,
  ...
  global_state->cache_eligible =
      cache_eval.eligible; // an eligible non-hit is a genuine miss (EXPLAIN)
  ...
  if (cache_eval.eligible) {
    ...
    Value sv;
    cap->max_entry_bytes =
        context.TryGetCurrentSetting("vgi_result_cache_max_entry_bytes", sv)
            ? static_cast<int64_t>(sv.GetValue<uint64_t>())
            : 67108864;
    cap->max_bytes =
        context.TryGetCurrentSetting("vgi_result_cache_max_bytes", sv)
            ? static_cast<int64_t>(sv.GetValue<uint64_t>())
            : 268435456;
    ...
    // max_entry_bytes AND the disk tier is on, capture spills to a streaming
    // disk blob (RAM-flat) instead of aborting. Captured here; the writer is
    // created lazily in InstallBatch on the first threshold cross.
#ifndef __EMSCRIPTEN__
    // WASM is memory-only: leave cap->disk_dir empty so a >max_entry_bytes
    // capture aborts to uncached (streams to DuckDB) rather than spilling to a
    // disk blob — the spill writer's incremental hash
    // (MbedTlsWrapper::SHA256State) is not resolvable from the emscripten
    // side-module. The singleton disk tier is gated in parallel at
    // VgiResultCache::Configure. Native is unchanged.
    if (context.TryGetCurrentSetting("vgi_result_cache_dir", sv) &&
        !sv.IsNull()) {
      cap->disk_dir = sv.GetValue<std::string>();
    }
#endif
    if (context.TryGetCurrentSetting("vgi_result_cache_disk_max_bytes", sv) &&
        !sv.IsNull()) {
      cap->disk_max = sv.GetValue<uint64_t>();
    }
    if (context.TryGetCurrentSetting("vgi_result_cache_disk_compression", sv) &&
        !sv.IsNull()) {
      cap->disk_compression = sv.GetValue<std::string>();
    }
    if (context.TryGetCurrentSetting("vgi_result_cache_disk_compression_level",
                                     sv) &&
        !sv.IsNull()) {
      cap->disk_compression_level = sv.GetValue<uint64_t>();
    }
    // Arm the per-partition split (SINGLE_VALUE_PARTITIONS). The actual split
    // at commit is ADDITIONALLY gated on the worker's first-batch
    // cc.partition_scope.
  ...
  if (revalidation_entry) {
    ...
    global_state->revalidate_if_modified_since =
        revalidation_entry->last_modified;
  ...
  auto perform_init_with_retry =
      [acquire_params, &context_ref = context, bind_result, projection_ids,
       filter_bytes, join_keys_buffers, order_by_hint = bind_data.order_by_hint,
       ...
       reval_ims = global_state->revalidate_if_modified_since](
          std::unique_ptr<IFunctionConnection> conn, bool from_pool) mutable
    ...
    auto run = [&](IFunctionConnection &c) {
      ...
      return c.PerformInit(bind_result, projection_ids, filter_bytes,
                           join_keys_buffers, "", order_by_hint,
                           table_sample_hint);
    };
    try {
      auto r = run(*conn);
      ...
      VGI_LOG(context_ref, "worker_pool.stale",
              {{"worker_path", worker_path},
               ...
               {"phase", "init_global"},
               {"error", e.what()}});
      auto fresh = AcquireConnectionForInit(context_ref, acquire_params,
                                            /*force_fresh=*/true);
  ...
  global_state->pending_init = duckdb::vgi::VgiWasmAsyncPool::Instance().Submit(
      [run = std::move(perform_init_with_retry), conn = std::move(connection),
       from_pool = acquired.from_pool]() mutable {
        return run(std::move(conn), from_pool);
      });
#else
  global_state->pending_init = std::async(
      std::launch::async,
      [run = std::move(perform_init_with_retry), conn = std::move(connection),
       from_pool = acquired.from_pool]() mutable {
        return run(std::move(conn), from_pool);
  ...
  auto applied =
      perform_init_with_retry(std::move(connection), acquired.from_pool);
...

  {
    ...
    fields.emplace_back("worker_path", bind_data.worker_path());
    fields.emplace_back("function_name", bind_data.function_name);
    fields.emplace_back("max_processes",
                        std::to_string(global_state->max_processes));
    fields.emplace_back("num_projection_columns",
                        std::to_string(projection_ids.size()));
    VGI_LOG(context, "table_function.init_global", fields);
  }

  if (bind_data.order_by_hint) {
    VGI_LOG(
        context, "table_function.order_pushdown",
        {{"function_name", bind_data.function_name},
  ...
  if (bind_data.table_sample_hint) {
    VGI_LOG(context, "table_function.sample_pushdown",
            {{"function_name", bind_data.function_name},
             {"percentage",
              std::to_string(bind_data.table_sample_hint->sample_percentage)},
...
VgiTableFunctionInitLocal(ExecutionContext &context,
  ...
  auto local_state = make_uniq<VgiTableFunctionLocalState>(
      std::move(current_chunk), context.client, bind_data.attach_params);
  ...
  if (bind_data.projection_pushdown) {
    ...
    // When a rowid column exists, remap column_ids to match the worker's full
    // schema indices. COLUMN_IDENTIFIER_ROW_ID maps to the actual worker column
    // index. Physical columns at or after the rowid position shift by +1. This
    // allows ArrowToDuckDB to look up the correct type info in
    // arrow_convert_data (which was built from the full worker schema including
    // the rowid column).
    if (bind_data.rowid_worker_col_index >= 0) {
  ...
  if (global_state.serving_from_cache) {
    ...
    std::shared_ptr<const VgiResultCacheEntry> entry =
        first ? global_state.serving_entry
              : std::make_shared<const VgiResultCacheEntry>();
    local_state->prefetch_slot_->connection =
        make_uniq<CachedReplayConnection>(std::move(entry));
  ...
  // Try to claim the primary connection from global_state (thread-safe
  // check-and-move)
  ...
  if (primary_connection) {
    // Primary worker: use connection from bind phase
    {
      ...
      fields.emplace_back("function_name", bind_data.function_name);
      fields.emplace_back("global_execution_id",
                          BytesToHex(global_state.global_execution_id));
    ...
    auto secondary_init = [&]() {
      ...
      local_state->connection()->SetTickFilterState(
          global_state.tick_filter_state);
      ...
      if (bind_data.supports_splits) {
        ...
        if (acquired.from_pool) {
          local_state->reacquire_fresh_connection =
              [params](ClientContext &ctx) mutable
              -> std::unique_ptr<IFunctionConnection> {
            auto fresh =
                AcquireConnectionForInit(ctx, params, /*force_fresh=*/true);
            return std::unique_ptr<IFunctionConnection>(
                fresh.connection.release());
          };
        }
        return;
      }
      ...
      if (bind_data.projection_pushdown) {
        local_state->connection()->PerformInit(
            secondary_bind_result, global_state.projection_ids,
            global_state.static_filter_bytes, global_state.join_keys_buffers,
            "", global_state.order_by_hint, global_state.table_sample_hint,
            global_state.init_opaque_data);
      } else {
        local_state->connection()->PerformInit(
            secondary_bind_result, {}, global_state.static_filter_bytes,
            global_state.join_keys_buffers, "", std::nullopt, std::nullopt,
            global_state.init_opaque_data);
      }
    };
    try {
      ...
      VGI_LOG(context.client, "worker_pool.stale",
              {{"worker_path", bind_data.worker_path()},
               ...
               {"error", e.what()}});
      acquired = AcquireConnectionForInit(context.client, params,
                                          /*force_fresh=*/true);
      local_state->prefetch_slot_->connection = std::move(acquired.connection);
      secondary_init();
    }
...
std::shared_ptr<arrow::Buffer> VgiSerializeDynamicFilterDelta(
  ...
  for (auto &update : updates) {
    ...
    yyjson_mut_obj_add_str(serializer.GetDoc(), obj, "operation",
                           remove ? "remove" : "upsert");
    yyjson_mut_obj_add_strcpy(serializer.GetDoc(), obj, "id",
                              update.predicate_id.c_str());
    yyjson_mut_obj_add_uint(serializer.GetDoc(), obj, "revision",
                            update.revision);
    if (!remove) {
      bool advisory = true;
      auto expression = serializer.SerializeFilter(
          *update.filter, update.column_index, update.column_name, advisory);
      if (!expression) {
        throw InvalidInputException(
            "VGI dynamic filter could not be serialized atomically");
      }
      yyjson_mut_obj_add_str(serializer.GetDoc(), obj, "mode", "advisory");
      yyjson_mut_obj_add_str(serializer.GetDoc(), obj, "source", "top_n");
      yyjson_mut_obj_add_val(serializer.GetDoc(), obj, "expression",
                             expression);
    }
    yyjson_mut_arr_append(json_updates, obj);
  }
  yyjson_mut_obj_add_val(serializer.GetDoc(), document, "updates",
                         json_updates);
  ...
  if (!imported.ok()) {
    throw IOException("Failed to import dynamic filter delta: %s",
  ...
  batch = arrow::RecordBatch::Make(arrow::schema(fields, metadata), 1,
                                   batch->columns());

  auto output_result = arrow::io::BufferOutputStream::Create();
  if (!output_result.ok()) {
    throw IOException("Failed to create dynamic filter delta buffer: %s",
  ...
  if (!writer_result.ok()) {
    throw IOException("Failed to create dynamic filter delta writer: %s",
                      writer_result.status().ToString());
  }
  auto writer = writer_result.ValueUnsafe();
  auto status = writer->WriteRecordBatch(*batch);
  if (!status.ok()) {
    throw IOException("Failed to write dynamic filter delta: %s",
                      status.ToString());
  }
  status = writer->Close();
  if (!status.ok()) {
    throw IOException("Failed to close dynamic filter delta: %s",
                      status.ToString());
  }
  auto finished = output->Finish();
  if (!finished.ok()) {
    throw IOException("Failed to finish dynamic filter delta: %s",
...
//! Called before each ReadDataBatch to ensure the tick carries the latest
//! filter.
static void
UpdateDynamicFilterState(VgiTableFunctionGlobalState &global_state,
  ...
  for (auto &df : global_state.dynamic_filters) {
    if (!df.filter_data->initialized.load()) {
      if (df.active) {
        df.revision++;
        df.active = false;
        updates.push_back({df.predicate_id, df.revision, df.column_index,
                           df.column_name, nullptr});
      }
      continue;
    }
    ...
    auto filter_copy = make_uniq<ConstantFilter>(const_filter.comparison_type,
                                                 const_filter.constant);
  ...
  try {
    auto bytes = VgiSerializeDynamicFilterDelta(
        context, bind_data.worker_path(), updates);
    global_state.tick_filter_state->encoded_filters =
        Blob::ToBase64(string_t(reinterpret_cast<const char *>(bytes->data()),
                                static_cast<idx_t>(bytes->size())));
...
//! ``duckdb/src/include/duckdb/parallel/pipeline.hpp:51``
//! (``BATCH_INCREMENT``). Returning a source-side index ``>= BATCH_INCREMENT -
//! 1`` triggers an
...
static duckdb::vector<ColumnPartitionData> ConvertPartitionValuesBatch(
  ...
  vector<LogicalType> types;
  vector<string> names;
  ArrowSchemaToDuckDBTypes(context, pv_batch->schema(), c_schema, arrow_table,
                           types, names);
  ...
  ArrowTableFunction::ArrowToDuckDB(
      local_scan_state, arrow_table.GetColumns(), chunk,
      /*arrow_scan_is_projected=*/false, COLUMN_IDENTIFIER_ROW_ID);
...
//! Decode a captured batch's vgi_partition_values bytes to the per-column MIN
//! values (row 0; == max for SINGLE_VALUE). Returns false on any decode/shape
//! problem (the caller then aborts the whole split — a batch we can't classify
//! must not be dropped).
static bool DecodePartitionMinTuple(ClientContext &context,
                                    const std::string &pv_bytes,
                                    std::vector<Value> &out) {
  if (pv_bytes.empty()) {
    return false;
  }
...
// Per-partition split-at-commit: bucket the captured batches by partition-value
// tuple and Insert one entry per distinct partition, keyed by (static key +
// residual filter) with a "p:" discriminator. Additive to the whole-scan entry.
// Best-effort — any decode failure or an over-cap partition count skips the
// split (the whole-scan entry stands).
static void SplitAndStorePartitionEntries(ClientContext *ctx,
                                          VgiResultCaptureCtx &cap,
                                          const VgiResultCacheEntry &meta,
                                          bool allow_disk) {
  ...
  std::map<std::string, Bucket>
      buckets; // "p:"-disc -> bucket (map = deterministic order)
  for (auto &sp : cap.streams) {
  ...
  for (auto &kv : buckets) {
    ...
    pe->scope = meta.scope;
    // v1: per-partition entries are non-revalidatable (the combined serve never
    // issues conditional requests). Freshness mirrors the whole-scan entry.
...
static bool InstallBatch(ClientContext &context,
                         const VgiTableFunctionBindData &bind_data,
  ...
  if (bind_data.supports_batch_index) {
    ...
    if (parsed_index >= VGI_BATCH_INDEX_CAP - 1) {
      throw IOException("VGI function '%s' emitted vgi_batch_index %llu that "
                        "exceeds DuckDB's "
                        "per-pipeline cap (%llu); choose a smaller index space",
                        bind_data.function_name,
                        static_cast<unsigned long long>(parsed_index),
  ...
  if (bind_data.partition_kind != VgiPartitionKind::NotPartitioned) {
    const std::string &pv_bytes =
        local_state.connection()->GetLastPartitionValuesBytes();
    if (pv_bytes.empty()) {
      throw IOException(
    ...
    if (!reader_result.ok()) {
      throw IOException(
          ...
          bind_data.function_name, reader_result.status().ToString());
    }
    ...
    if (static_cast<idx_t>(pv_batch->num_columns()) !=
        declared_indices.size()) {
      throw IOException(
    ...
    for (idx_t i = 0; i < declared_indices.size(); ++i) {
      ...
      if (!pv_field->type()->Equals(*declared_field->type())) {
        throw IOException(
            ...
            declared_field->type()->ToString(), pv_field->type()->ToString());
      }
    }
    ...
    if (!bind_data.supports_batch_index) {
      local_state.current_batch_index =
          global_state.synthetic_batch_index.fetch_add(
  ...
  // Reset() clears owned_data in array_states, which is REQUIRED so that
  // ArrowToDuckDB will update owned_data to point to the new chunk. Without
  // this, owned_data still points to the previous chunk, and when that chunk is
  // released, the data becomes invalid.
  local_state.Reset();
  ...
  const auto batch_bytes =
      static_cast<idx_t>(arrow::util::TotalBufferSize(*arrow_batch));
  ...
  if (global_state.capture && local_state.capture_stream) {
    ...
    if (!cap.aborted.load(std::memory_order_relaxed)) {
      ...
      auto spill_append = [&](const std::shared_ptr<arrow::Buffer> &ipc,
                              int64_t logical_len) -> bool {
        ...
        return cap.disk_writer->AppendBatch(
            ...
            static_cast<int64_t>(arrow_batch->num_rows()),
            conn->GetLastPartitionValuesBytes(), ipc->data(),
            static_cast<int64_t>(ipc->size()), logical_len);
      };
      auto log_disk_too_large = [&]() {
        cap.aborted.store(true, std::memory_order_relaxed);
        VgiResultCache::Instance().NoteCaptureAbort();
        LogResultCache(
            context, "result_cache.abort",
            {{"key_hash", cap.key.HexDigest()},
      ...
      if (not_cacheable) {
        ...
        if (!local_state.capture_spilled) {
          if (!DrainSubstreamToWriter(*local_state.capture_stream,
                                      *cap.disk_writer)) {
            log_disk_too_large();
          }
        ...
        if (!cap.aborted.load(std::memory_order_relaxed)) {
          // Compress AT SOURCE (the live batch is here) — one serialize pass,
          // no transcode round-trip on the multi-GB spill path. logical_len is
          // the uncompressed IPC size (cheap, no extra serialize).
          //
          // NOTE (bytes= unit): this uses GetRecordBatchSize (the RecordBatch
          // IPC message size) whereas the memory-capture path (and the
          // pre-threshold spill drain) uses
          // SerializeRecordBatch(uncompressed)->size(), which also counts any
          // IPC stream framing. So the ref's logical bytes= can differ by a few
          // hundred bytes/batch of framing between a spilled-live entry and a
          // memory-persisted one. Deliberately NOT unified: matching it would
          // add an uncompressed serialize to this hot path, or shift reported
          // total_bytes for every memory entry — not worth it for a
          // sub-batch-of-framing delta. The disk byte CAP uses the on-disk
          // COMPRESSED size (consistent), so only the advisory logical bytes= /
          // materialize-vs-stream threshold is affected.
          int64_t logical_len = 0;
          if (!arrow::ipc::GetRecordBatchSize(*arrow_batch, &logical_len)
                   .ok()) {
            logical_len = 0;
          }
          auto ipc = SerializeRecordBatch(arrow_batch, nullptr,
                                          cap.disk_writer->CodecName(),
        ...
        const int64_t running =
            cap.total_bytes.fetch_add(sz, std::memory_order_relaxed) + sz;
        if (running > cap.max_entry_bytes) {
          ...
          if (!cap.disk_dir.empty() && cap.disk_max > 0) {
            std::lock_guard<std::mutex> lk(cap.mu);
            if (!cap.disk_writer &&
                !cap.aborted.load(std::memory_order_relaxed)) {
              cap.disk_writer =
                  VgiResultCache::Instance().BeginStreamingCapture(
...
static void MaybeSlideRevalidatedEntry(
    ClientContext &context, const VgiTableFunctionBindData &bind_data,
    VgiTableFunctionGlobalState &global_state, const VgiCacheControl &cc) {
  const auto &orig = *global_state.revalidation_entry;
  auto fresh = std::make_shared<VgiResultCacheEntry>(
      orig); // shallow copy (shared Buffers)
  ...
  if (ttl <= 0 && cc.expires_rfc3339.empty() && !orig.never_expires) {
    auto prior = std::chrono::duration_cast<std::chrono::seconds>(
                     orig.expires_at - orig.stored_at)
                     .count();
    ttl = prior > 0 ? prior : 0;
  }
  if (!orig.never_expires) {
    fresh->expires_at =
        fresh->stored_at + std::chrono::seconds(ttl > 0 ? ttl : 0);
  }
  ...
  LogResultCache(
      context, "result_cache.revalidate",
...
//! cost. `estimated_bytes` is a nullable hint a worker may never populate, and
//! for a worker doing per-split inference or mixing warm cache with cold object
//! storage it is uncorrelated with time. Any client-side grouping would be a
//! guess about costs invisible to it.
//!
//! `fetch_add` is ALSO what keeps `batch_index` monotone per reader: it hands
//! out strictly ascending indices, and the worker derives batch_index from the
//! split's position in a globally-ordered space. Nothing else enforces that.
static bool AdvanceToNextSplit(ClientContext &context,
                               const VgiTableFunctionBindData &bind_data,
                               VgiTableFunctionGlobalState &global_state,
                               VgiTableFunctionLocalState &local_state) {
  if (!bind_data.supports_splits) {
  ...
  // from `next_split` because a claim consumed by a reader that then died is
  // NOT a drained split, and conflating them would let a partial result commit.
  ...
  if (had_split) {
    ...
    if (global_state.capture) {
      global_state.capture->completed_splits.fetch_add(
          1, std::memory_order_relaxed);
    }
  ...
  const idx_t idx =
      global_state.next_split.fetch_add(1, std::memory_order_relaxed);
  ...
  auto retry_stale_pooled = [&](const IOException &e) {
    ...
    VGI_LOG(context, "worker_pool.stale",
            {{"worker_path", bind_data.worker_path()},
             ...
             {"error", e.what()}});
    local_state.prefetch_slot_->connection =
        local_state.reacquire_fresh_connection(context);
  ...
  auto do_split_init = [&]() {
    auto &conn = *local_state.connection();
    try {
      // Only reset when there IS a prior stream. On this reader's first claim
      // the connection was merely acquired (InitLocal is acquire-only on the
      // split path), so there is nothing to close or drain yet.
      if (had_split) {
      ...
      // Poison HERE. The comment used to claim this had already happened, and
      // it had not: the only thing setting the flag was a failed input-writer
      // close inside ResetForNextSplit, so an init that threw — an interrupt, a
      // partial write, a worker that died between reset and response — left the
...
static bool GetNextBatch(ClientContext &context,
                         const VgiTableFunctionBindData &bind_data,
                         VgiTableFunctionGlobalState &global_state,
                         VgiTableFunctionLocalState &local_state) {
  if (local_state.done) {
  ...
  // acquires a connection on this path — there is no primary init to mirror —
  // so without this the first ReadDataBatch would fire against an uninitialized
  // connection.
  //
  ...
  if (bind_data.supports_splits && !global_state.serving_from_cache &&
      !local_state.has_split) {
    if (!AdvanceToNextSplit(context, bind_data, global_state, local_state)) {
      ...
      return InstallBatch(context, bind_data, global_state, local_state,
                          nullptr);
    }
  }
    ...
    if (arrow_batch->num_rows() == 0) {
      ...
      if (global_state.revalidating && global_state.revalidation_entry &&
        ...
        if (cc.not_modified) {
          ...
          local_state.prefetch_slot_->connection =
              make_uniq<CachedReplayConnection>(
                  global_state.revalidation_entry);
          if (global_state.capture) {
...
static void LaunchPrefetch(TableFunctionInput &input,
                           VgiTableFunctionLocalState &local_state) {
...
static void ConsumePrefetchedBatch(ClientContext &context,
                                   const VgiTableFunctionBindData &bind_data,
                                   VgiTableFunctionGlobalState &global_state,
                                   VgiTableFunctionLocalState &local_state) {
  ...
  InstallBatch(context, bind_data, global_state, local_state,
               std::move(arrow_batch));
}
...
static void ConvertCurrentBatch(const VgiTableFunctionBindData &bind_data,
  ...
  if (output_size > 0) {
    idx_t rowid_col = (!bind_data.projection_pushdown &&
                       bind_data.rowid_worker_col_index >= 0)
                          ? static_cast<idx_t>(bind_data.rowid_worker_col_index)
    ...
    for (idx_t idx = 0; idx < output.ColumnCount(); idx++) {
      idx_t col_idx =
          local_state.column_ids.empty() ? idx : local_state.column_ids[idx];
      ...
      if (static_cast<int64_t>(arrow_array_idx) >= n_children) {
        throw IOException("VGI function '%s' emitted a batch with %lld "
                          "column(s) but the scan needs column "
                          "index %llu; the worker's scan-time output is "
                          "inconsistent with its declared bind "
                          "output schema",
                          bind_data.function_name,
                          static_cast<long long>(n_children),
...
void VgiTableFunctionScan(ClientContext &context, TableFunctionInput &input,
  ...
  bool is_async = (input.results_execution_mode ==
                   AsyncResultsExecutionMode::TASK_EXECUTOR);
  if (is_async) {
    Value async_val;
  ...
  if (state == PrefetchState::READY) {
    ...
    ConvertCurrentBatch(bind_data, global_state, local_state, output);
    // Guard: if DuckDB-side filtering empties the chunk, IMPLICIT + empty =
    // FINISHED. Signal HAVE_MORE_OUTPUT to prevent premature termination.
    if (output.size() == 0) {
  ...
  // IN_FLIGHT: should be impossible — DuckDB won't call scan while task is
  // running
  D_ASSERT(state != PrefetchState::IN_FLIGHT);
  throw InternalException(
      "VgiTableFunctionScan: unexpected prefetch state IN_FLIGHT");
}
...
VgiTableFunctionCardinality(ClientContext &context,
  ...
  // Lazy fetch: make a single table_function_cardinality RPC call on first
  // invocation
  if (!bind_data.cardinality_fetched &&
      !bind_data.bind_result.bind_request_bytes.empty()) {
    bind_data.cardinality_fetched = true;
    try {
      auto rpc_params =
          bind_data.attach_params
              ? bind_data.attach_params
              : std::make_shared<VgiAttachParameters>(
                    bind_data.worker_path(), "", bind_data.worker_debug(),
                    bind_data.use_pool());
      CatalogRpcContext rpc_ctx{rpc_params, bind_data.attach_opaque_data,
                                bind_data.transaction_opaque_data};
      auto result = InvokeTableFunctionCardinality(
          rpc_ctx, bind_data.bind_result.bind_request_bytes,
  ...
  VGI_LOG(
      context, "table_function.cardinality",
      {{"worker_path", bind_data.worker_path()},
       {"function_name", bind_data.function_name},
       {"cardinality_estimate",
        have_est ? std::to_string(bind_data.cardinality_estimate) : "unknown"},
       {"cardinality_max",
        have_max ? std::to_string(bind_data.cardinality_max) : "unknown"}});

  if (have_est && have_max) {
    // NodeStatistics(idx_t estimated, idx_t max) sets both flags.
    return make_uniq<NodeStatistics>(
        static_cast<idx_t>(bind_data.cardinality_estimate),
...
double
VgiTableFunctionProgress(ClientContext &context,
                         const FunctionData *bind_data_p,
  ...
  if (bind_data.cardinality_estimate > 0) {
    ...
    double progress = (static_cast<double>(rows_read) /
                       static_cast<double>(bind_data.cardinality_estimate)) *
                      100.0;
...
VgiTableFunctionToString(TableFunctionToStringInput &input) {
  ...
  if (bind_data.order_by_hint) {
    ...
    if (bind_data.order_by_hint->row_limit >= 0) {
      result["Row Limit Hint"] =
          std::to_string(bind_data.order_by_hint->row_limit);
    }
  }
  if (bind_data.table_sample_hint) {
    result["Sample Hint"] =
...
VgiTableFunctionDynamicToString(TableFunctionDynamicToStringInput &input) {
  ...
  // ANALYZE should say so (and which tier). The worker RPC below is also
  // skipped on a hit (no global_execution_id), so surface this before the early
  // return.
  if (global_state.serving_from_cache) {
    result["Cache"] =
        (global_state.serving_entry && global_state.serving_entry->disk_backed)
            ? "hit (disk_streaming)"
            : "hit (memory)";
  ...
  const auto batches =
      global_state.batches_received.load(std::memory_order_relaxed);
  if (batches > 0) {
    const auto rows =
        global_state.batch_rows_total.load(std::memory_order_relaxed);
    const auto rows_min =
        global_state.batch_rows_min.load(std::memory_order_relaxed);
    const auto rows_max =
        global_state.batch_rows_max.load(std::memory_order_relaxed);
    const auto bytes =
        global_state.batch_bytes_total.load(std::memory_order_relaxed);
    result["Batches"] =
        StringUtil::Format("%llu (rows: min %llu, avg %llu, max %llu)",
  ...
  // context to make the call — happens for direct vgi_table_function()
  // invocations before bind has populated bind_request_bytes, or if InitGlobal
  // wasn't reached (cached ClientContext is null).
  if (bind_data.bind_result.bind_request_bytes.empty() ||
      global_state.global_execution_id.empty() ||
  ...
  try {
    ...
    auto user_map = InvokeTableFunctionDynamicToString(
        rpc_ctx, bind_data.bind_result.bind_request_bytes,
        bind_data.bind_result.opaque_data, global_state.global_execution_id,
        *global_state.client_context_for_explain);
    ...
    VGI_LOG(*global_state.client_context_for_explain,
            "table_function.dynamic_to_string_error",
            {{"worker_path", bind_data.worker_path()},
             {"function_name", bind_data.function_name},
             {"error", e.what()}});
  }
...
VgiTableScanGetVirtualColumns(ClientContext &context,
  ...
  result.insert(
      {COLUMN_IDENTIFIER_ROW_ID, TableColumn("rowid", bind_data.rowid_type)});
  return result;
}
...
void VgiSetScanOrder(unique_ptr<RowGroupOrderOptions> order_options,
  ...
  bind_data.order_by_hint =
      OrderByHint{bind_data.all_column_names[col_idx], std::move(direction),
                  std::move(null_order), row_limit};
}
...
VgiGetPartitionData(ClientContext &, TableFunctionGetPartitionInput &input) {
  auto &local_state = input.local_state->Cast<VgiTableFunctionLocalState>();
  auto &bind_data = input.bind_data->Cast<VgiTableFunctionBindData>();
  ...
  if (!local_state.current_partition_data.empty() &&
    ...
    result.partition_data.reserve(
        input.partition_info.partition_columns.size());
    for (column_t requested_col : input.partition_info.partition_columns) {
      ...
      result.partition_data.push_back(
          local_state.current_partition_data[declared_pos]);
    }
  }
  return result;
}
...
// GROUP BY planning.
TablePartitionInfo VgiGetPartitionInfo(ClientContext &,
...
//   1. Catalog-declared stats via VgiTableEntry::GetStatistics (table_entry
//   path).
//      These are cached on the table entry and shared across queries.
//   2. Function-level stats via table_function_statistics RPC (direct path).
//   These
...
FetchFunctionStatistics(ClientContext &context,
                        const VgiTableFunctionBindData &bind_data,
  ...
  if (!bind_data.statistics_fetched) {
    ...
    try {
      ...
      bind_data.statistics_cache = InvokeTableFunctionStatistics(
          rpc_ctx, bind_data.bind_result.bind_request_bytes,
          bind_data.bind_result.opaque_data, bind_data.all_column_types,
          bind_data.all_column_names, bind_data.function_name, context);
    } catch (const std::exception &e) {
...
VgiTableFunctionStatistics(ClientContext &context,
  ...
  return FetchFunctionStatistics(context, bind_data,
                                 bind_data.all_column_names[column_index]);
}
