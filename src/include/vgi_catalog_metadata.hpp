...
struct VgiSecretTypeParam {
  ...
  LogicalType
      type; // Value type — any DuckDB type (VARCHAR, INTEGER, BOOLEAN, etc.)
...
struct VgiAttachCatalogInfo {
  std::string
      alias; // ATTACH alias; also a catalog-table branch's source_catalog
  std::string
      target; // ATTACH target — path or DSN (e.g. "ducklake:sqlite:/m.sqlite")
  std::string
      db_type; // DuckDB db type; empty = infer from target scheme prefix
  ...
  std::string secret_ref; // Optional credential to pre-resolve via Orchard;
                          // empty = catch-all
};
...
struct CatalogAttachResult {
  ...
  std::vector<VgiSecretType>
      secret_types; // Secret types exposed by this catalog
  std::vector<VgiAttachCatalogInfo>
      attach_catalogs; // Companion catalogs to ATTACH at VGI-attach time
  std::string comment; // Optional comment describing this catalog
  std::map<std::string, std::string>
      tags; // Optional key-value tags for this catalog
  bool supports_column_statistics =
      false; // Whether any tables provide column statistics
...
struct VgiSchemaInfo {
  ...
  // VgiCatalogSet::CacheKindName()
  // ("table", "view", "scalar_function", "aggregate_function",
  // "table_function", "macro", "index"). Empty when the worker doesn't populate
  // the field; missing keys mean "no opinion" — the eager-load threshold treats
  // them as 1, biasing toward bulk LoadEntries() for unspecified populations.
  std::map<std::string, int64_t> estimated_object_count;
};

// Result of table_scan_function_get - tells DuckDB which function to call to
// scan a table This enables catalogs to delegate scanning to any DuckDB
// function (e.g., read_parquet, iceberg_scan)
struct VgiScanFunctionResult {
  std::string
      function_name; // The DuckDB function to call (e.g., "read_parquet")
  duckdb::vector<Value>
      positional_arguments; // Positional arguments for the function
  std::map<std::string, Value>
      named_arguments; // Named arguments for the function
  std::vector<std::string>
      required_extensions; // Extensions to load before calling
  // DuckDB 1.5 schema containing function_name; adapted from VGI 2.0
  // schema_path. Empty when the worker doesn't know (pre-1.5.0 peer) or the
  // function is a native DuckDB function with no VGI-side schema of its own
  // (e.g. read_parquet) — callers fall back to the pre-1.5.0
  // table-schema/default-schema heuristic in that case.
...
struct VgiScanBranch {
  std::string
      function_name; // e.g. "vgi_table_scan", "iceberg_scan", "read_parquet"
  ...
  // the format's registered reader (parquet, csv, iceberg, ...), so a worker
  // can hand back "these 40 parquet files" without needing a function per
  // format or knowing the reader's argument spelling.
  ...
  bool IsFunctionBranch() const { return !function_name.empty(); }

  // Function branch only: DuckDB 1.5 schema containing function_name, adapted
  // from schema_path. Empty when registered in. Empty for a
  // catalog-table/format branch, a pre-1.5.0 peer, or a native DuckDB function
  // — not to be confused with source_schema above, which names a catalog-table
  // branch's SOURCE table's schema, a different, older field.
  std::string schema_name;
};
...
struct VgiScanBranchesResult {
  std::vector<VgiScanBranch>
      branches; // One or more; empty list is loud-rejected at parse
...
struct VgiTableInfo {
  ...
  // Column statistics capability — indicates this table can provide
...
struct VgiMacroInfo {
  ...
  std::vector<uint8_t>
      parameter_default_values_bytes; // IPC-serialized defaults batch
  ...
  // VgiFunctionInfo::arguments_schema. Null on older workers that don't emit
  // it.
...
// Wire format: lowercase string ("scalar", "table", "table_buffering",
// "aggregate")
...
//   "PRESERVES_ORDER"    -> OrderPreservationType::INSERTION_ORDER (DuckDB
//   default) "NO_ORDER_GUARANTEE" -> OrderPreservationType::NO_ORDER
...
struct VgiScanPlan {
  ...
  //! trading round trips for a smaller peak; seeing the count is how an
  //! operator tells "one page" from "thirty" without instrumenting the worker.
  idx_t pages = 0;
};
...
struct VgiFunctionInfo {
  ...
  // Uses DuckDB's FunctionStability enum (CONSISTENT, VOLATILE,
  // CONSISTENT_WITHIN_QUERY)
  std::optional<FunctionStability> stability;
  // Uses DuckDB's FunctionNullHandling enum (DEFAULT_NULL_HANDLING,
  // SPECIAL_HANDLING)
  ...
  // divided into named, independently redeemable splits. Read at BIND (not
  // init) so a ``SET vgi_split_scans`` between bind and execute cannot produce
  // a bind that assumed splits and an init that does not.
  ...
  // streaming scan - but the field is on the wire so retrofitting it later
  // would not re-shape plan().
  ...
  // extension functions and runtime artifacts are capability records of their
  // own.
  ...
  // placeholder), so ONE registration serves f(52,13) (literal), FROM t,
  // f(t.x,t.y) (columns), and LATERAL f(t.x,t.y). The registration enters the
  // in-out branch on this flag even without a TABLE-typed arg, and bind builds
  // the input schema from the declared arg names + drives the literal
...
// The bytes contain a single-row RecordBatch with: name, description, type,
// default_value
...
// The bytes contain a single-row RecordBatch with: name, description,
// parameters_schema parameters_schema is an IPC-serialized Arrow schema where
// each field defines a secret parameter Field metadata {"redact": "true"} marks
// keys that should be redacted in SHOW SECRETS
...
// Parse a VgiTableInfo from an Arrow RecordBatch at a specific row (for
// multi-row batches)
VgiTableInfo ParseTableInfo(ClientContext &context,
                            const std::shared_ptr<arrow::RecordBatch> &batch,
...
// The arguments field is a nested IPC batch with arg_0, arg_1, ... for
// positional args and named args by name
