...
//! unknown catalog version, or a secret dependency with no fingerprint). On true,
//! fills `key`, `catalog_name`, `catalog_version`.
//!
//! A secret-dependent function is keyed on `secret_scope` (BindResult::secret_scope
//! of the bind whose secrets the worker holds), so a changed secret misses.
//!
