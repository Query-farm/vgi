...
ReadBatch(const std::shared_ptr<arrow::Buffer> &bytes) {
...
void RequireV2Schema(const std::shared_ptr<arrow::RecordBatch> &batch) {
  ...
  REQUIRE(
      batch->schema()->metadata()->Get("vgi_filter_encoding").ValueOrDie() ==
      "vgi.filters.v2");
  REQUIRE(batch->schema()->metadata()->Get("vgi_filter_version").ValueOrDie() ==
          "2");
  REQUIRE(
      batch->schema()->metadata()->Get("vgi_evaluation_context").ValueOrDie() ==
      "vgi.none.v1");
...
TEST_CASE("filter v2 snapshot uses schema metadata and unprojected column refs",
          "[filter-v2]") {
  DuckDB db(nullptr);
  Connection con(db);
  TableFilterSet filters;
  filters.filters[0] = make_uniq<ConstantFilter>(
      ExpressionType::COMPARE_GREATERTHAN, Value::INTEGER(41));

  auto serialized = VgiSerializeFilters(*con.context, {1}, &filters,
                                        {"ignored", "actual"}, "test-worker");
...
TEST_CASE("filter v2 maps every comparison operator", "[filter-v2]") {
  ...
  for (auto &entry : cases) {
    ...
    TableFilterSet filters;
    filters.filters[0] =
        make_uniq<ConstantFilter>(entry.type, Value::INTEGER(1));
    auto json = FilterSpec(ReadBatch(
        VgiSerializeFilters(*con.context, {0}, &filters, {"n"}, "test-worker")
            .filter_bytes));
    REQUIRE(json.find(string("\"op\":\"") + entry.name + "\"") !=
            std::string::npos);
  }
}
...
TEST_CASE("filter v2 maps null predicates", "[filter-v2]") {
  ...
  for (bool negated : {false, true}) {
    ...
    REQUIRE(json.find(negated ? "\"negated\":true" : "\"negated\":false") !=
            std::string::npos);
  }
}

TEST_CASE("filter v2 preserves arbitrarily nested field_ref nodes",
          "[filter-v2]") {
  DuckDB db(nullptr);
  Connection con(db);
  TableFilterSet filters;
  auto leaf =
      make_uniq<ConstantFilter>(ExpressionType::COMPARE_EQUAL, Value("x"));
  auto inner = make_uniq<StructFilter>(2, "leaf", std::move(leaf));
  filters.filters[0] = make_uniq<StructFilter>(1, "middle", std::move(inner));

  auto serialized =
      VgiSerializeFilters(*con.context, {0}, &filters, {"root"}, "test-worker");
...
TEST_CASE("filter v2 makes the filter-key index domain explicit",
          "[filter-v2]") {
  DuckDB db(nullptr);
  Connection con(db);
  TableFilterSet filters;
  filters.filters[2] = make_uniq<ConstantFilter>(ExpressionType::COMPARE_EQUAL,
                                                 Value::INTEGER(7));
  auto direct = VgiSerializeFilters(*con.context, {}, &filters, {"a", "b", "c"},
                                    "test-worker", "", -1, nullptr,
  ...
  REQUIRE_THROWS_AS(VgiSerializeFilters(*con.context, {0}, &filters,
                                        {"a", "b", "c"}, "test-worker"),
                    InvalidInputException);

  TableFilterSet rowid_filters;
  rowid_filters.filters[0] = make_uniq<ConstantFilter>(
      ExpressionType::COMPARE_EQUAL, Value::BIGINT(9));
  auto rowid =
      VgiSerializeFilters(*con.context, {COLUMN_IDENTIFIER_ROW_ID},
                          &rowid_filters, {"a", "b"}, "test-worker", "rid", 1);
  ...
  shifted_filters.filters[0] = make_uniq<ConstantFilter>(
      ExpressionType::COMPARE_EQUAL, Value::INTEGER(7));
  auto shifted = VgiSerializeFilters(*con.context, {1}, &shifted_filters,
                                     {"a", "b"}, "test-worker", "rid", 0);
...
TEST_CASE("filter v2 chooses exact inline and external IN encodings",
          "[filter-v2]") {
  DuckDB db(nullptr);
  Connection con(db);

  TableFilterSet small_filters;
  small_filters.filters[0] =
      make_uniq<InFilter>(vector<Value>{Value::INTEGER(1), Value::INTEGER(2)});
  auto small = VgiSerializeFilters(*con.context, {0}, &small_filters, {"key"},
                                   "test-worker");
  ...
  REQUIRE(FilterSpec(small_batch).find("\"kind\":\"literal\"") !=
          std::string::npos);
  ...
  auto large = VgiSerializeFilters(*con.context, {0}, &large_filters, {"key"},
                                   "test-worker");
  ...
  REQUIRE(
      keys->schema()->metadata()->Get("vgi_join_keys_version").ValueOrDie() ==
      "2");
  ...
  auto empty = VgiSerializeFilters(*con.context, {0}, &empty_filters, {"key"},
                                   "test-worker");
  ...
  REQUIRE(
      FilterSpec(empty_batch).find("\"node\":\"literal\",\"value_ref\":0") !=
      std::string::npos);
}

TEST_CASE("required IN exceeding the transport limit fails closed",
          "[filter-v2]") {
  DuckDB db(nullptr);
  Connection con(db);
  DBConfig::GetConfig(*con.context)
      .AddExtensionOption("vgi_join_keys_max_bytes", "test limit",
                          LogicalType::UBIGINT, Value::UBIGINT(0));
  REQUIRE_FALSE(con.Query("SET vgi_join_keys_max_bytes=1")->HasError());
  TableFilterSet filters;
  filters.filters[0] =
      make_uniq<InFilter>(vector<Value>{Value::BIGINT(1), Value::BIGINT(2)});
  REQUIRE_THROWS_AS(
      VgiSerializeFilters(*con.context, {0}, &filters, {"key"}, "test-worker"),
      InvalidInputException);
}
...
TEST_CASE("required OR serialization is all or decline", "[filter-v2]") {
  ...
  disjunction->child_filters.push_back(make_uniq<ConstantFilter>(
      ExpressionType::COMPARE_EQUAL, Value::INTEGER(1)));
...
TEST_CASE("required throwing AND remains one atomic predicate", "[filter-v2]") {
  ...
  auto narrowing = BoundCastExpression::AddCastToType(
      *con.context, std::move(reference), LogicalType::TINYINT);
  auto comparison = make_uniq<BoundComparisonExpression>(
      ExpressionType::COMPARE_GREATERTHAN, std::move(narrowing),
  ...
  conjunction->child_filters.push_back(
      make_uniq<ExpressionFilter>(std::move(comparison)));
  conjunction->child_filters.push_back(make_uniq<ConstantFilter>(
      ExpressionType::COMPARE_NOTEQUAL, Value::BIGINT(0)));
  TableFilterSet filters;
...
TEST_CASE("filter v2 encodes advisory mode and canonical type references",
          "[filter-v2]") {
  DuckDB db(nullptr);
  Connection con(db);
  TableFilterSet filters;
  auto reference = make_uniq<BoundReferenceExpression>(LogicalType::INTEGER, 0);
  auto cast = BoundCastExpression::AddCastToType(
      *con.context, std::move(reference), LogicalType::BIGINT);
  auto comparison = make_uniq<BoundComparisonExpression>(
      ExpressionType::COMPARE_EQUAL, std::move(cast),
      make_uniq<BoundConstantExpression>(Value::BIGINT(42)));
  filters.filters[0] = make_uniq<OptionalFilter>(
      make_uniq<ExpressionFilter>(std::move(comparison)));
  ...
  REQUIRE(json.find("\"mode\":\"advisory\",\"source\":\"other\"") !=
          std::string::npos);
}

TEST_CASE("filter v2 tick delta carries revisioned upsert and remove",
          "[filter-v2]") {
  DuckDB db(nullptr);
  Connection con(db);
  auto filter = make_uniq<ConstantFilter>(
      ExpressionType::COMPARE_LESSTHANOREQUALTO, Value::INTEGER(99));
  vector<VgiDynamicFilterDeltaUpdate> updates{
      {"top_n:0", 3, 0, "score", filter.get()},
      {"top_n:1", 4, 1, "other", nullptr}};

  auto batch = ReadBatch(
      VgiSerializeDynamicFilterDelta(*con.context, "test-worker", updates));
  RequireV2Schema(batch);
  auto json = FilterSpec(batch);
  REQUIRE(json.find("\"kind\":\"delta\"") != std::string::npos);
  REQUIRE(
      json.find("\"operation\":\"upsert\",\"id\":\"top_n:0\",\"revision\":3") !=
      std::string::npos);
  REQUIRE(json.find("\"mode\":\"advisory\",\"source\":\"top_n\"") !=
          std::string::npos);
  REQUIRE(
      json.find("\"operation\":\"remove\",\"id\":\"top_n:1\",\"revision\":4") !=
      std::string::npos);
}
