...
pub extern "C" fn vgi_rust_serve_table_sab_slot(slot: i32) {
    ...
    // No set_catalog: the dispatcher installs a default CatalogModel. The C++
    // client names the owning schema on the bind (as the catalog path does), so
    // the function resolves as `main.count_to` against that default model.
